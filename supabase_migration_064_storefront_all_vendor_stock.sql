-- Storefront vendor stock for ALL vendors, not just Adidas.
--
-- webstore_storefront_products previously read live vendor (drop-ship) stock and
-- ETAs from adidas_inventory only. But the catalog spans Under Armour, Nike,
-- Agron, SanMar, S&S, Momentec, Richardson too — so a non-Adidas item showed zero
-- vendor availability (its sizes looked out of stock, and checkout's stock guard,
-- which reads this same view, could wrongly block an orderable size). It could
-- also pull the WRONG brand's stock when a SKU happened to collide across vendors.
--
-- Fix: read from inventory_unified (the UNION ALL of every vendor's inventory),
-- matched on BOTH sku AND ai.source = p.inventory_source. The source match:
--   • brings in every vendor we sync (UA, Nike, Agron, SanMar, S&S, …),
--   • prevents cross-brand SKU collisions (same SKU, different vendor),
--   • guarantees one row per (sku,size) so jsonb_object_agg can't collide.
-- products.inventory_source uses the same vocabulary as inventory_unified.source
-- (click/ua/nike/agron/sanmar/ss_activewear/momentec/richardson), and Adidas
-- products are 'click', so the existing Adidas behavior is preserved exactly.
--
-- Only the vendor LATERAL join changes; every column is reproduced verbatim from
-- the live definition. The canonical full definition lives at
-- supabase/views/webstore_storefront_products.sql — edit there going forward.
--
