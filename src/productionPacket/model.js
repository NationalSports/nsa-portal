...
export function buildProductionPacket({ store, orders = [], lines = [], salesOrders = [], catalog = [], notes = [], messages = [], soId = null }) {
  ...
  const players = reconciled.lines.map((l, i) => {
    ...
    return { id: `${text(l.id || 'extra')}:${text(l._effSku || l.sku)}:${text(l._size || l.size)}:${i}`, orderKey: text(l.order_id || 'extra'), orderId: text(order.order_number || order.omg_order_number || l.order_id), soId: text(l._sourceSoId), player: text(l.player_name || 'Unassigned'), number: text(l.player_number), sku: text(l._effSku || l._sku || l.sku), name: text(l._name || l.name || catalog.find(c=>c.product_id===l.product_id)?.display_name), color: text(l._color || l.color), size: text(l._size || l.size), qty: qty(l.qty), wasSku: text(l._wasSku), wasSize: text(l._wasSize), unbatched: !l._sourceSoId, verify: !!(l._verify || l._unmatched || l._sizeVerify), extra: !!l._orderExtra };
  });
