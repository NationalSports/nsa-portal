  ...
  const {cust,sos,invs,prod,vend,invAdjLog=[],invPOs,submittedBatches,qbApi,qbConfig,persistQbLink,nf,dP,
    ...
    const syncPurchaseOrders=async(prodQBMapArg={},options={})=>{
      ...
      for(const group of poGroups){
        ...
        if(qbVendorId){
          const savedVendor=existingQBVendors.find(row=>String(row.Id)===String(qbVendorId));
          const savedNames=[savedVendor?.DisplayName,savedVendor?.CompanyName].filter(Boolean);
          if(!savedVendor||!savedNames.includes(vendorName)){const error='saved QBO vendor ID is missing or does not exactly match "'+vendorName+'"';log.details.push(group.poId+' — BLOCKED: '+error);results.push({poId:group.poId,result:'blocked',error});log.status='partial';continue}
        }
        if(!qbVendorId){
        ...
        if(exact.length===1&&sameNumber.length===1){
          try{const verified=await verifyPOReadback(exact[0].Id);
            await persistQbLink({mapKey:'qbPOMap',sourceIds:[group.poId],qboId:verified.Id,log:{...log,details:[group.poId+' — linked and verified QBO PO #'+verified.Id]},evidence:{result:'linked',api_readback:true,doc_number:group.poId,vendor_id:qbVendorId,date:poDate,total:totalAmount,line_count:qbLines.length}});
        ...
        if(res?.PurchaseOrder?.Id){
          try{
            ...
            await persistQbLink({mapKey:'qbPOMap',sourceIds:[group.poId],qboId:verified.Id,log:{...log,details:[group.poId+' — created and verified QBO PO #'+verified.Id]},evidence:{result:'created',api_readback:true,doc_number:group.poId,vendor_id:qbVendorId,date:poDate,total:totalAmount,line_count:qbLines.length}});
