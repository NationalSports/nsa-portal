...
export default function FinancialsPage() {
  ...
  // 'ready' = finished work you can invoice; 'check' = old orders still open (status check, not billable yet)
  ...
  const staleRows = stale.rows.filter((r) => {
    if (staleMode === 'ready' ? !r.invoiceable : r.invoiceable) return false;
  ...
  const exportStale = () => downloadCsv((staleMode === 'ready' ? 'ready-to-invoice-' : 'check-status-orders-') + today.toLocaleDateString('en-CA') + '.csv',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {tab === 'stale' && (
        <>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <Tile label="Ready to invoice" value={$k(stale.summary.value)} sub={stale.summary.invoiceableCount + ' orders · everything shipped or billed'} subColor={stale.summary.invoiceableCount ? WARN : GOOD} />
          ...
          <div style={card}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 420px' }}>
                <h2 style={S.h2}>{staleMode === 'ready' ? 'Ready to invoice' : 'Check status \u2014 still open, not ready to invoice'}</h2>
                <div style={{ fontSize: 11.5, color: INK2, maxWidth: 850 }}>
                  {staleMode === 'ready'
                    ? <>Uninvoiced orders whose work is finished: shipped, marked complete, or every production job done. Finished jobs are allowed through
                      even when receiving or shipping data is incomplete &mdash; those rows are marked as a likely system mismatch instead of being hidden.
                      Promo orders and orders marked \u201cno invoice needed\u201d are excluded.</>
            ...
            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginTop: 12, marginBottom: 10, alignItems: 'center' }}>
              {[['ready', 'Ready to invoice ' + stale.summary.invoiceableCount], ['check', 'Check status ' + stale.summary.oldCount]].map(([id, label]) =>
              ...
              {(staleMode === 'ready' ? [
                ['all', 'All ' + stale.summary.invoiceableCount], ['critical', 'Critical'],
                ['ready', 'Ready ' + stale.summary.readyCount], ['system_mismatch', 'Mismatch ' + stale.summary.mismatchCount],
              ] : [['all', 'All ' + stale.summary.oldCount], ['critical', 'Critical']]).map(([id, label]) => <button key={id} onClick={() => setStaleFilter(id)} style={{
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 1120 }}>
                <thead><tr>
                  ...
                  <th style={th}>{staleMode === 'ready' ? 'Open to invoice' : 'Uninvoiced value'}</th><th style={{ ...th, textAlign: 'left' }}>Why it is here</th>
                </tr></thead>
                <tbody>
                  {staleRows.map((r) => {
              ...
              {!staleRows.length && <div style={{ padding: 24, textAlign: 'center', color: INK2 }}>{staleMode === 'ready' ? 'No finished orders are waiting to be invoiced.' : 'No old open orders match these filters.'}</div>}
            </div>
          </div>
