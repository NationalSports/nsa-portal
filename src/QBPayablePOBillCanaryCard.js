...
export default function QBPayablePOBillCanaryCard(){
  ...
  async function request(action){
    ...
    try{
      ...
      const body=action==='execute'?{action,approved:true,previewHash:preview.previewHash}:action==='repair'?{action,approved:true,qboBillId:repair.qboBillId}:{action:'preview'};
      ...
      if(!response.ok){if(data.repairable&&data.qboBillId&&data.candidate)setRepair(data);throw new Error((data.error||'PO-to-bill canary unavailable')+(data.details?' · '+JSON.stringify(data.details):''))}
      if(action==='execute'||action==='repair'||data.status==='complete')setResult(data);else setPreview(data);
  ...
  return <section aria-label="PO-linked bill canary" style={{marginTop:18}}>
    ...
    {repair&&!result&&<div style={{marginTop:10,padding:10,border:'2px solid #b91c1c',borderRadius:6}}>
      <p><strong>Approved correction:</strong> partial QBO Bill #{repair.qboBillId} · {money(repair.candidate.freight+repair.candidate.sportsFee)}. Its replacement must verify at {money(repair.candidate.total)} and close QBO PO #{repair.candidate.qboPurchaseOrderId} before the partial bill is deleted.</p>
      <button className="btn btn-sm" disabled={busy} onClick={()=>request('repair')}>Create verified replacement, then delete partial bill</button>
    </div>}
    {result&&<p role="status">Verified QBO Bill #{result.qboBillId} linked to QBO Purchase Order #{result.qboPurchaseOrderId}. {result.repaired?`Partial Bill #${result.replacedQboBillId} deleted. `:''}Durable receipts saved.</p>}
