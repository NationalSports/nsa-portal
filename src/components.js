...
// ─── BIN LOCATION PICKER ───
// Warehouse put-away grid: 5 shelving units (A–E) in a row, 4 bays each — bays numbered
...
// Two-step: tap a unit letter first, then the bay/shelf spots — selecting is optional.
// Rendered through a portal to <body> so page stacking contexts can't cover it or
// swallow its clicks (the warehouse page nests it deep inside cards/sticky tables).
const WH_UNITS=5, WH_BAYS_PER_UNIT=4, WH_SHELVES=5;
const WH_UNIT_LETTERS=['A','B','C','D','E'];
...
const BinLocationPicker=({title,subtitle,onSave,onSkip})=>{
  ...
  const[unit,setUnit]=useState(null);// null = unit chooser (step 1); 0-4 = bay/shelf grid for that unit (step 2)
  const toggle=(code)=>setSel(p=>p.includes(code)?p.filter(x=>x!==code):[...p,code]);
  const unitOfBay=(bay)=>Math.min(WH_UNITS-1,Math.floor((bay-1)/WH_BAYS_PER_UNIT));
  const selCountIn=(u)=>sel.filter(c=>unitOfBay(parseInt(c))===u).length;
  const baysOf=(u)=>Array.from({length:WH_BAYS_PER_UNIT},(_,i)=>u*WH_BAYS_PER_UNIT+i+1);
  const overlay=
  <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.55)',zIndex:100000,display:'flex',alignItems:'center',justifyContent:'center',padding:10}} onClick={onSkip}>
    <div onClick={e=>e.stopPropagation()} style={{background:'white',borderRadius:16,maxWidth:640,width:'100%',maxHeight:'94vh',display:'flex',flexDirection:'column',boxShadow:'0 24px 64px rgba(0,0,0,0.35)'}}>
      <div style={{padding:'14px 20px',background:'linear-gradient(135deg,#155e75,#0891b2)',color:'white',borderRadius:'16px 16px 0 0'}}>
        ...
        <div style={{fontSize:11,opacity:0.8,marginTop:6}}>{unit==null?'Tap the shelf unit it went on — or Skip.':'Tap every bay/shelf spot these units went. Multiple allowed.'}</div>
      </div>
      <div style={{padding:16,overflow:'auto',flex:1,WebkitOverflowScrolling:'touch'}}>
        {unit==null?
          /* Step 1 — pick a shelf unit */
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(104px,1fr))',gap:10}}>
            {WH_UNIT_LETTERS.map((L,u)=>{const n=selCountIn(u);const bays=baysOf(u);
              return<button type="button" key={L} onClick={()=>setUnit(u)}
                style={{position:'relative',minHeight:104,borderRadius:14,cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:4,
                  border:n>0?'2px solid #0e7490':'2px solid #cbd5e1',background:n>0?'#ecfeff':'white'}}>
                <span style={{fontSize:34,fontWeight:900,color:'#0e7490',lineHeight:1}}>{L}</span>
                <span style={{fontSize:11,fontWeight:700,color:'#64748b'}}>Bays {bays[0]}–{bays[bays.length-1]}</span>
                {n>0&&<span style={{position:'absolute',top:8,right:8,minWidth:22,height:22,borderRadius:11,background:'#0891b2',color:'white',fontSize:12,fontWeight:800,display:'flex',alignItems:'center',justifyContent:'center',padding:'0 6px'}}>{n}</span>}
              </button>;})}
          </div>
        :
          /* Step 2 — bay × shelf grid for the chosen unit */
          <>
            <div style={{display:'flex',gap:6,alignItems:'center',marginBottom:12,flexWrap:'wrap'}}>
              <button type="button" onClick={()=>setUnit(null)} style={{minHeight:40,padding:'0 12px',borderRadius:8,border:'1px solid #cbd5e1',background:'white',color:'#475569',fontWeight:700,fontSize:13,cursor:'pointer'}}>← Units</button>
              {WH_UNIT_LETTERS.map((L,u)=>{const n=selCountIn(u);const on=u===unit;
                return<button type="button" key={L} onClick={()=>setUnit(u)}
                  style={{minWidth:46,minHeight:40,borderRadius:8,fontWeight:800,fontSize:15,cursor:'pointer',
                    border:on?'2px solid #0e7490':'1px solid #cbd5e1',background:on?'#0891b2':n>0?'#ecfeff':'white',color:on?'white':'#475569'}}>
                  {L}{n>0?<span style={{fontSize:10,fontWeight:800,marginLeft:3,verticalAlign:'top'}}>{n}</span>:null}</button>;})}
            </div>
            <div style={{display:'grid',gridTemplateColumns:'24px repeat('+WH_BAYS_PER_UNIT+',minmax(56px,1fr))',gap:6,alignItems:'stretch'}}>
              <div/>
              {baysOf(unit).map(bay=><div key={'h'+bay} style={{textAlign:'center',fontSize:10,fontWeight:800,color:'#64748b',textTransform:'uppercase'}}>Bay {bay}</div>)}
              {Array.from({length:WH_SHELVES},(_,si)=>{
                const shelf=WH_SHELVES-si;// shelf 5 (top) first, 1 (bottom) last — mirrors the physical rack
                return<React.Fragment key={shelf}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:800,color:'#94a3b8'}}>{shelf}</div>
                  {baysOf(unit).map(bay=>{
                    const code=bay+'-'+shelf;const on=sel.includes(code);
                    return<button type="button" key={code} onClick={()=>toggle(code)} title={binLabel(code)}
                      style={{minHeight:52,borderRadius:10,cursor:'pointer',fontSize:14,fontWeight:800,padding:0,
            ...
            <div style={{marginTop:8,fontSize:10,color:'#94a3b8',textAlign:'center'}}>Shelf 1 = bottom · Shelf 5 = top</div>
          </>}
      </div>
      <div style={{padding:'12px 16px',borderTop:'1px solid #e2e8f0',background:'#f8fafc',borderRadius:'0 0 16px 16px'}}>
        ...
        <div style={{display:'flex',gap:8}}>
          <button type="button" onClick={onSkip} style={{flex:1,maxWidth:160,padding:12,borderRadius:10,border:'1px solid #cbd5e1',background:'white',color:'#64748b',fontWeight:700,fontSize:14,cursor:'pointer',minHeight:48}}>Skip</button>
          <button type="button" disabled={sel.length===0} onClick={()=>sel.length>0&&onSave(sel)}
