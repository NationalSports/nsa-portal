...
{id:'c1',parent_id:null,name:'Orange Lutheran High School',alpha_tag:'OLu',contacts:[{name:'Athletic Director',email:'athletics@orangelutheran.org',phone:'714-555-0100',role:'Primary'},{name:'Janet Wu',email:'jwu@orangelutheran.org',phone:'714-555-0109',role:'Accounting'}],billing_address_line1:'2222 N Santiago Blvd',billing_city:'Orange',billing_state:'CA',billing_zip:'92867',shipping_address_line1:'2222 N Santiago Blvd',shipping_city:'Orange',shipping_state:'CA',shipping_zip:'92867',adidas_ua_tier:'A',catalog_markup:1.65,payment_terms:'net30',tax_rate:0.0775,primary_rep_id:'r1',is_active:true,_open_estimates:1,_open_sos:2,_open_invoices:1,_open_balance:4200},
{id:'c1a',parent_id:'c1',name:'OLu Baseball',alpha_tag:'OLuB',contacts:[{name:'Coach Martinez',email:'martinez@orangelutheran.org',phone:'',role:'Coach'}],shipping_city:'Orange',shipping_state:'CA',adidas_ua_tier:'A',catalog_markup:1.65,payment_terms:'net30',primary_rep_id:'r1',is_active:true,_open_estimates:0,_open_sos:1,_open_invoices:1,_open_balance:4200},
{id:'c1b',parent_id:'c1',name:'OLu Football',alpha_tag:'OLuF',contacts:[{name:'Coach Davis',email:'davis@orangelutheran.org',phone:'',role:'Coach'}],shipping_city:'Orange',shipping_state:'CA',adidas_ua_tier:'A',catalog_markup:1.65,payment_terms:'net30',primary_rep_id:'r1',is_active:true,_open_estimates:1,_open_sos:1,_open_invoices:0,_open_balance:0},
{id:'c1c',parent_id:'c1',name:'OLu Track & Field',alpha_tag:'OLuT',contacts:[{name:'Coach Chen',email:'chen@orangelutheran.org',phone:'',role:'Coach'}],shipping_city:'Orange',shipping_state:'CA',adidas_ua_tier:'A',catalog_markup:1.65,payment_terms:'net30',primary_rep_id:'r1',is_active:true,_open_estimates:0,_open_sos:0,_open_invoices:0,_open_balance:0},
{id:'c2',parent_id:null,name:'St. Francis High School',alpha_tag:'SF',contacts:[{name:'AD Office',email:'ad@stfrancis.edu',phone:'818-555-0200',role:'Primary'}],billing_city:'La Canada',billing_state:'CA',shipping_city:'La Canada',shipping_state:'CA',adidas_ua_tier:'B',catalog_markup:1.65,payment_terms:'net30',tax_rate:0.095,primary_rep_id:'r2',is_active:true,_open_estimates:0,_open_sos:1,_open_invoices:2,_open_balance:6800},
{id:'c2a',parent_id:'c2',name:'St. Francis Lacrosse',alpha_tag:'SFL',contacts:[{name:'Coach Resch',email:'resch@stfrancis.edu',phone:'',role:'Coach'}],shipping_city:'La Canada',shipping_state:'CA',adidas_ua_tier:'B',catalog_markup:1.65,payment_terms:'net30',primary_rep_id:'r2',is_active:true,_open_estimates:0,_open_sos:1,_open_invoices:2,_open_balance:6800},
...
{id:'c3a',parent_id:'c3',name:'Clovis High Badminton',alpha_tag:'CHBad',contacts:[{name:'Coach Kim',email:'kim@clovisusd.k12.ca.us',phone:'',role:'Coach'}],shipping_city:'Clovis',shipping_state:'CA',adidas_ua_tier:'B',catalog_markup:1.65,payment_terms:'prepay',primary_rep_id:'r3',is_active:true,_open_estimates:2,_open_sos:0,_open_invoices:0,_open_balance:0},
];
...
{id:'v2',name:'Under Armour',vendor_type:'upload',nsa_carries_inventory:true,click_automation:true,invoice_scan_enabled:true,is_active:true,contact_email:'teamdealer@underarmour.com',contact_phone:'888-727-6687',rep_name:'Mike Daniels',payment_terms:'net60',notes:'',_open_invoices:2,_invoice_total:8200,_aging_current:5200,_aging_30:3000,_aging_60:0,_aging_90:0},
{id:'v3',name:'SanMar',vendor_type:'api',api_provider:'sanmar',nsa_carries_inventory:false,is_active:true,contact_email:'orders@sanmar.com',contact_phone:'800-426-6399',payment_terms:'net30',notes:'API connected. Auto-inventory.',_open_invoices:1,_invoice_total:2100,_aging_current:2100,_aging_30:0,_aging_60:0,_aging_90:0},
...
{id:'INV-1039',type:'invoice',customer_id:'c2a',date:'02/01/26',total:3400,paid:0,days:17,memo:'Lacrosse Store Batch 1',status:'open'},
];
...
function CustomerModal({isOpen,onClose,onSave,customer,parents}){
  const blank={parent_id:null,name:'',alpha_tag:'',contacts:[{name:'',email:'',phone:'',role:'Primary'}],shipping_address_line1:'',shipping_city:'',shipping_state:'',shipping_zip:'',billing_address_line1:'',billing_city:'',billing_state:'',billing_zip:'',adidas_ua_tier:'B',catalog_markup:1.65,payment_terms:'net30',notes:''};
  ...
  const validate=()=>{const e={};if(!form.name)e.name='Required';if(!form.alpha_tag)e.alpha_tag='Required';
    ...
    const contacts=form.contacts||[];if(contacts.length===0||!contacts[0]?.name)e.contact_name='At least one contact required';
    if(contacts.length===0||!contacts[0]?.email)e.contact_email='Email required';
  ...
  <div className="modal-body">
    ...
    {custType==='sub'&&<div className="form-group" style={{marginBottom:12}}><label className="form-label">Parent Customer *{errors.parent_id&&<span style={{color:'#dc2626',marginLeft:4}}>{errors.parent_id}</span>}</label>
      <SearchSelect options={parents.map(p=>({value:p.id,label:`${p.name} (${p.alpha_tag})`}))} value={form.parent_id} onChange={v=>set('parent_id',v)} placeholder="Search for parent customer..."/></div>}
    ...
    <div style={{fontSize:11,fontWeight:700,color:'#64748b',marginTop:12,marginBottom:6,textTransform:'uppercase',display:'flex',alignItems:'center',justifyContent:'space-between'}}>Contacts {errors.contact_name&&<span style={{color:'#dc2626',fontWeight:400,textTransform:'none'}}>{errors.contact_name}</span>}</div>
    {(form.contacts||[]).map((c,i)=><div key={i} style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr auto',gap:8,marginBottom:6,alignItems:'end'}}>
      <div><input className="form-input" placeholder="Name *" value={c.name} onChange={e=>updateContact(i,'name',e.target.value)} style={i===0&&errors.contact_name&&!c.name?{borderColor:'#dc2626'}:{}}/></div>
      <div><input className="form-input" placeholder="Email *" value={c.email} onChange={e=>updateContact(i,'email',e.target.value)} style={i===0&&errors.contact_email&&!c.email?{borderColor:'#dc2626'}:{}}/></div>
      <div style={{display:'flex',gap:4}}><input className="form-input" placeholder="Phone" value={c.phone} onChange={e=>updateContact(i,'phone',e.target.value)} style={{flex:1}}/><input className="form-input" placeholder="Role" value={c.role} onChange={e=>updateContact(i,'role',e.target.value)} style={{width:80}}/></div>
      {i>0&&<button className="btn btn-sm btn-secondary" onClick={()=>removeContact(i)} style={{padding:'4px 6px'}}><Icon name="trash" size={12}/></button>}
    </div>)}
    ...
    <div className="form-row form-row-2">
      <div className="form-group"><label className="form-label">Adidas / UA Tier</label><select className="form-select" value={form.adidas_ua_tier||'B'} onChange={e=>set('adidas_ua_tier',e.target.value)}><option value="A">A - 40% off retail</option><option value="B">B - 35% off retail</option><option value="C">C - 30% off retail</option><option value="custom">Custom %</option></select></div>
      <div className="form-group"><label className="form-label">Catalog Markup (default 1.65x, rounds to nearest $0.25)</label><input className="form-input" type="number" step="0.05" value={form.catalog_markup||1.65} onChange={e=>set('catalog_markup',parseFloat(e.target.value)||1.65)}/></div>
    </div>
  </div>
  <div className="modal-footer"><button className="btn btn-secondary" onClick={onClose}>Cancel</button><button className="btn btn-primary" onClick={()=>{if(!validate())return;onSave({...form,id:form.id||'c'+Date.now(),is_active:true,_open_estimates:form._open_estimates||0,_open_sos:form._open_sos||0,_open_invoices:form._open_invoices||0,_open_balance:form._open_balance||0});onClose()}}>{customer?.id?'Save':'Create'}</button></div>
  </div></div>);
}

function CustomerDetail({customer,allCustomers,allOrders,onBack,onEdit,onSelectCustomer}){
  const[tab,setTab]=useState('activity');const[orderFilter,setOrderFilter]=useState('all');
  ...
  const tierLabel={A:'40% off retail',B:'35% off retail',C:'30% off retail'};
  ...
  <div style={{flex:1}}>
    <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}><span style={{fontSize:20,fontWeight:800}}>{customer.name}</span><span className="badge badge-blue">{customer.alpha_tag}</span><span className="badge badge-green">Tier {customer.adidas_ua_tier}</span><span className="badge badge-gray">{tl[customer.payment_terms]||'Net 30'}</span><span className="badge badge-gray">{customer.catalog_markup||1.65}x catalog</span></div>
    <div style={{fontSize:13,color:'#64748b',marginTop:4}}>{(customer.contacts||[]).map((c,i)=><span key={i}>{c.name}{c.email&&` (${c.email})`}{i<customer.contacts.length-1&&' | '}</span>)}{customer.shipping_city&&<span> | {customer.shipping_city}, {customer.shipping_state}</span>}</div>
  ...
  {subs.map(s=><div key={s.id} style={{padding:'10px 18px',borderBottom:'1px solid #f1f5f9',display:'flex',alignItems:'center',gap:8,cursor:'pointer'}} onClick={()=>onSelectCustomer(s)}>
    <span style={{color:'#cbd5e1'}}>|_</span><span style={{fontWeight:600,color:'#1e40af'}}>{s.name}</span><span className="badge badge-gray">{s.alpha_tag}</span>
    <span style={{fontSize:11,color:'#94a3b8'}}>{(s.contacts||[])[0]?.name}</span><div style={{flex:1}}/>
  ...
  {tab==='activity'&&<div className="card"><div className="card-header"><h2>Orders & Invoices</h2><div style={{display:'flex',gap:4}}>
    {[['all','All'],['estimate','Estimates'],['sales_order','Sales Orders'],['invoice','Invoices']].map(([v,l])=><button key={v} className={`btn btn-sm ${orderFilter===v?'btn-primary':'btn-secondary'}`} onClick={()=>setOrderFilter(v)}>{l}</button>)}</div></div>
    ...
    {filteredOrders.length===0?<tr><td colSpan={7} style={{textAlign:'center',color:'#94a3b8',padding:20}}>No records found</td></tr>:
  ...
  {tab==='overview'&&<div className="card"><div className="card-header"><h2>Customer Information</h2></div><div className="card-body">
    <div className="form-row form-row-3">
      <div><div className="form-label">Billing Address</div><div style={{fontSize:13}}>{customer.billing_address_line1||'--'}<br/>{customer.billing_city}, {customer.billing_state} {customer.billing_zip}</div></div>
      ...
      <div><div className="form-label">Tax Rate</div><div style={{fontSize:13}}>{customer.tax_rate?(customer.tax_rate*100).toFixed(2)+'%':'Auto'}</div></div>
    </div>
    <div style={{marginTop:16}}><div className="form-label">Pricing</div><div style={{fontSize:13}}>Adidas/UA: Tier {customer.adidas_ua_tier} ({tierLabel[customer.adidas_ua_tier]||'Custom'})<br/>Catalog: {customer.catalog_markup||1.65}x from cost (rounded to $0.25)</div></div>
  ...
  {tab==='reporting'&&<div className="card"><div className="card-header"><h2>Sales Reporting</h2></div><div className="card-body">
    <div className="stats-row"><div className="stat-card"><div className="stat-label">Lifetime Revenue</div><div className="stat-value">$48,200</div></div><div className="stat-card"><div className="stat-label">This Year</div><div className="stat-value">$15,600</div></div><div className="stat-card"><div className="stat-label">Last Year</div><div className="stat-value">$32,600</div></div><div className="stat-card"><div className="stat-label">Avg Order</div><div className="stat-value">$4,820</div></div></div>
    <div style={{color:'#94a3b8',fontSize:13,textAlign:'center',padding:20}}>Monthly chart and sub-customer revenue breakdown will populate with real data</div>
  </div></div>}
  </div>);
}
...
function VendorDetail({vendor,onBack,onEdit}){
  ...
  <div className="card"><div className="card-header"><h2>Purchase Orders</h2></div><div className="card-body"><div className="empty">PO tracking will be available in Phase 2</div></div></div>
  </div>);
}
...
export default function App() {
  ...
  const filteredProducts = useMemo(() => {
    let list = products;
    if (search&&page==='products') { const q=search.toLowerCase(); list=list.filter(p=>p.sku.toLowerCase().includes(q)||p.name.toLowerCase().includes(q)||p.brand?.toLowerCase().includes(q)||p.color?.toLowerCase().includes(q)); }
    ...
    if (productFilter.color!=='all') list=list.filter(p=>p.color===productFilter.color);
    return list;
  ...
  const inventoryData = useMemo(() => {
    ...
    if (invFilter.color!=='all') list=list.filter(p=>p.color===invFilter.color);
  ...
  const stockAlerts=useMemo(()=>{const alerts=[];products.forEach(p=>{if(!p._alerts)return;Object.entries(p._alerts).forEach(([sz,min])=>{const cur=p._inv?.[sz]||0;if(cur<min)alerts.push({product:p,size:sz,current:cur,minimum:min,needed:min-cur})})});return alerts},[products]);
  ...
  const renderDashboard = () => (<>
    <div className="stats-row">
      ...
      <div className="stat-card"><div className="stat-label">Inventory Value</div><div className="stat-value">${totalInvValue.toLocaleString(undefined,{maximumFractionDigits:0})}</div><div className="stat-sub">{totalInvUnits} units in warehouse</div></div>
      <div className="stat-card" style={stockAlerts.length>0?{borderColor:'#fbbf24'}:{}}><div className="stat-label">Stock Alerts</div><div className="stat-value" style={{color:stockAlerts.length>0?'#d97706':''}}>{stockAlerts.length}</div><div className="stat-sub">{stockAlerts.length>0?'sizes below minimum':'all stocked'}</div></div>
    </div>
    {stockAlerts.length>0&&<div className="card" style={{marginBottom:16,borderLeft:'4px solid #d97706'}}><div className="card-header"><h2 style={{color:'#d97706'}}><Icon name="alert" size={16}/> Stock Alerts</h2><button className="btn btn-sm btn-primary" onClick={()=>notify('Draft restock PO generated! (Phase 2)')}>Auto-Generate Restock PO</button></div>
      <div className="card-body" style={{padding:0}}><table><thead><tr><th>SKU</th><th>Product</th><th>Size</th><th>Current</th><th>Minimum</th><th>Need</th></tr></thead><tbody>
    ...
    <div className="card" style={{marginBottom:16}}><div className="card-header"><h2>Quick Actions</h2></div><div className="card-body" style={{display:'flex',gap:8,flexWrap:'wrap'}}>
      ...
      <button className="btn btn-secondary" onClick={()=>setCsvModal({open:true,type:'Customers',fields:['name','alpha_tag','parent_name','contact_name','contact_email','pricing_tier','payment_terms']})}><Icon name="upload" size={14}/> Import Customers</button>
    </div></div>
  ...
  const renderCustomers = () => {
    ...
    const filtered=parentCustomers.filter(p=>{
      ...
      if(search){const q=search.toLowerCase();return p.name.toLowerCase().includes(q)||getChildren(p.id).some(c=>c.name.toLowerCase().includes(q))}return true;
    });
    return(<>
      ...
      {filtered.map(parent=>{
        const kids=getChildren(parent.id);const bal=kids.reduce((a,c)=>a+(c._open_balance||0),parent._open_balance||0);
        ...
        return(<div key={parent.id} className="card" style={{marginBottom:10}}>
          <div style={{padding:'12px 16px',display:'flex',alignItems:'center',gap:12}}>
            ...
            <button className="btn btn-sm btn-primary" onClick={e=>{e.stopPropagation();setCustModal({open:true,customer:{parent_id:parent.id,adidas_ua_tier:parent.adidas_ua_tier,catalog_markup:parent.catalog_markup,payment_terms:parent.payment_terms}})}}><Icon name="plus" size={12}/> Sub</button>
          </div>
  ...
  const renderVendors = () => {
    if (selectedVendor) return <VendorDetail vendor={selectedVendor} onBack={()=>setSelectedVendor(null)} onEdit={v=>{notify('Vendor edit modal coming in next update')}}/>;
  ...
  const renderProducts = () => (<>
    <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap'}}>
      ...
      <select className="form-select" style={{width:120}} value={productFilter.stock} onChange={e=>setProductFilter(f=>({...f,stock:e.target.value}))}><option value="all">All Products</option><option value="instock">In Stock</option></select>
    ...
    {filteredProducts.map(p=>{
      ...
      const isAdidas=p.brand==='Adidas'||p.brand==='Under Armour';
        ...
        <div style={{display:'flex',gap:14,alignItems:'flex-start'}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:12,color:'#94a3b8',marginTop:2}}>
              ...
              <span style={{marginLeft:8,color:'#166534',fontWeight:600}}>Sell: ${isAdidas?'Tier based':roundQ(p.nsa_cost*1.65).toFixed(2)}</span>
              {currentUser.role==='admin'&&<button className="btn btn-sm btn-secondary" style={{marginLeft:8,padding:'1px 6px',fontSize:10}}><Icon name="edit" size={10}/> Edit</button>}
            </div>
            <div style={{display:'flex',gap:2,marginTop:8,flexWrap:'wrap',alignItems:'center'}}>
              {p.available_sizes.map(s=>{const q=p._inv?.[s]||0;const al=p._alerts?.[s];const belowAlert=al&&q<al;return(<div key={s} className={`size-cell ${q>10?'in-stock':q>0?(belowAlert?'alert-stock':'low-stock'):'no-stock'}`}><div className="size-label">{s}</div><div className="size-qty">{q}</div>{belowAlert&&<div style={{fontSize:7,color:'#dc2626'}}>min:{al}</div>}</div>)})}
  ...
  const renderInventory = () => (<>
    <div className="stats-row">
      ...
      <div className="stat-card" style={stockAlerts.length>0?{borderColor:'#fbbf24'}:{}}><div className="stat-label">Stock Alerts</div><div className="stat-value" style={{color:stockAlerts.length>0?'#d97706':''}}>{stockAlerts.length}</div>
  ...
  return(<div className="app">
    ...
    <div className="main">
      <div className="topbar"><h1>{selectedCustomer?selectedCustomer.name:selectedVendor?selectedVendor.name:(titles[page]||'Dashboard')}</h1><div style={{fontSize:12,color:'#94a3b8'}}>Phase 1 v2.1</div></div>
