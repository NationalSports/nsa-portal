...
const STATUS_COLORS={waiting_art:{bg:'#fef3c7',color:'#92400e'},in_production:{bg:'#dbeafe',color:'#1e40af'},ready_ship:{bg:'#dcfce7',color:'#166534'},shipped:{bg:'#ede9fe',color:'#6d28d9'},completed:{bg:'#f1f5f9',color:'#475569'}};
...
{id:'v3',name:'SanMar',vendor_type:'api',api_provider:'sanmar',nsa_carries_inventory:false,is_active:true,contact_email:'orders@sanmar.com',payment_terms:'net30',notes:'API connected.',_open_invoices:1,_invoice_total:2100,_aging_current:2100,_aging_30:0,_aging_60:0,_aging_90:0},
...
{id:'p7',vendor_id:'v5',sku:'112',name:'Richardson Trucker Cap',brand:'Richardson',color:'Black/White',category:'Hats',retail_price:12,nsa_cost:4.5,available_sizes:['OSFA'],is_active:true,_inv:{OSFA:50},_alerts:{OSFA:20},_colors:['Black/White','Navy/White','Red/White','Royal/White','Charcoal/White']},
...
{id:'SO-1051',customer_id:'c2a',memo:'Lacrosse Team Store - Jan 2026',status:'in_production',created_by:'r2',created_at:'02/14/26 10:30 AM',updated_at:'02/15/26',expected_date:'03/10/26',production_notes:'',shipping_type:'flat',shipping_value:0,ship_to_id:'default',firm_dates:[],art_files:[{id:'af3',name:'SFL_Lacrosse_Crest.eps',notes:'',status:'uploaded',uploaded:'02/15/26'}],items:[]},
];
...
// Get all addresses for a customer (self + parent + siblings)
function getAddresses(cust, allCust){
  const addrs=[];const add=(c,label)=>{if(c.shipping_address_line1||c.shipping_city)addrs.push({id:c.id,label:`${label}: ${c.shipping_address_line1||''} ${c.shipping_city||''}, ${c.shipping_state||''}`.trim(),addr:`${c.shipping_address_line1||''} ${c.shipping_city||''}, ${c.shipping_state||''} ${c.shipping_zip||''}`.trim()})};
  if(!cust)return addrs;add(cust,'Default');
  if(cust.parent_id){const par=allCust.find(x=>x.id===cust.parent_id);if(par){add(par,par.alpha_tag);allCust.filter(c=>c.parent_id===par.id&&c.id!==cust.id).forEach(sib=>add(sib,sib.alpha_tag))}}
  else{allCust.filter(c=>c.parent_id===cust.id).forEach(sub=>add(sub,sub.alpha_tag))}
  if(cust.billing_address_line1)addrs.push({id:'billing',label:`Billing: ${cust.billing_address_line1} ${cust.billing_city||''}, ${cust.billing_state||''}`,addr:`${cust.billing_address_line1} ${cust.billing_city||''}, ${cust.billing_state||''} ${cust.billing_zip||''}`});
  return addrs;
}

// UNIFIED ORDER EDITOR - works for both Estimates and Sales Orders
function OrderEditor({order,mode,customer:ic,allCustomers,products,onSave,onBack,onConvertSO,currentUser,onNotify}){
  const isEst=mode==='estimate';const isSO=mode==='so';
  const[o,setO]=useState(order);const[cust,setCust]=useState(ic);const[pSearch,setPSearch]=useState('');const[showAdd,setShowAdd]=useState(false);const[saved,setSaved]=useState(!!order.customer_id);
  const[tab,setTab]=useState('items');const[newAddr,setNewAddr]=useState('');const[showNewAddr,setShowNewAddr]=useState(false);
  ...
  const selCust=id=>{const c=allCustomers.find(x=>x.id===id);if(c){setCust(c);sv('customer_id',id);sv('default_markup',c.catalog_markup||1.65)}};
  const addProd=p=>{const au=isAU(p.brand);const sell=au?rQ(p.retail_price*(1-(tD[cust?.adidas_ua_tier||'B']||0.35))):rQ(p.nsa_cost*(o.default_markup||1.65));
    sv('items',[...o.items,{product_id:p.id,sku:p.sku,name:p.name,brand:p.brand,color:p.color,nsa_cost:p.nsa_cost,retail_price:p.retail_price,unit_sell:sell,available_sizes:p.available_sizes,_colors:p._colors||null,sizes:{},decorations:[]}]);setShowAdd(false);setPSearch('')};
  ...
  const addD=i=>{const it=o.items[i];uI(i,'decorations',[...it.decorations,{type:'screen_print',position:'Front Center',colors:1,stitches:8000,underbase:false,two_color:false,dtf_size:0,sell_override:null,art_group:null,art_file_id:null}])};
  ...
  const addArt=()=>{const af=o.art_files||[];sv('art_files',[...af,{id:'af'+Date.now(),name:'New_File_'+(af.length+1)+'.ai',notes:'',status:'uploaded',uploaded:new Date().toLocaleDateString()}])};
  const uArt=(i,k,v)=>sv('art_files',(o.art_files||[]).map((f,x)=>x===i?{...f,[k]:v}:f));
  const rmArt=i=>sv('art_files',(o.art_files||[]).filter((_,x)=>x!==i));

  const addrs=useMemo(()=>getAddresses(cust,allCustomers),[cust,allCustomers]);
  ...
  const fp=products.filter(p=>{if(!pSearch)return true;const qq=pSearch.toLowerCase();return p.sku.toLowerCase().includes(qq)||p.name.toLowerCase().includes(qq)||p.brand?.toLowerCase().includes(qq)});
  const artFiles=o.art_files||[];
  const allArtAssigned=o.items.every(it=>it.decorations.every(d=>d.art_file_id));
  const statusFlow=isSO?['waiting_art','in_production','ready_ship','shipped','completed']:[];
  const si=statusFlow.indexOf(o.status);

  return(<div>
    <button className="btn btn-secondary" onClick={onBack} style={{marginBottom:12}}><Icon name="back" size={14}/> {isEst?'All Estimates':'All Sales Orders'}</button>
    {/* HEADER */}
    <div className="card" style={{marginBottom:16}}><div style={{padding:'16px 20px'}}>
      <div style={{display:'flex',gap:16,alignItems:'flex-start',flexWrap:'wrap'}}>
        <div style={{flex:1,minWidth:300}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4,flexWrap:'wrap'}}><span style={{fontSize:22,fontWeight:800,color:'#1e40af'}}>{o.id}</span>
            {isEst&&<span className={`badge ${o.status==='draft'?'badge-gray':o.status==='sent'?'badge-amber':o.status==='approved'?'badge-green':'badge-blue'}`}>{o.status}</span>}
            {isSO&&<span style={{padding:'3px 10px',borderRadius:12,fontSize:12,fontWeight:700,background:STATUS_COLORS[o.status]?.bg||'#f1f5f9',color:STATUS_COLORS[o.status]?.color||'#475569'}}>{o.status?.replace(/_/g,' ')}</span>}
            {isEst&&<EmailBadge est={o}/>}</div>
          {!cust?<div style={{marginBottom:8}}><label className="form-label">Select Customer *</label><SearchSelect options={allCustomers.map(c=>({value:c.id,label:`${c.name} (${c.alpha_tag})`}))} value={o.customer_id} onChange={selCust} placeholder="Search customer..."/></div>
          :<div><div style={{fontSize:18,fontWeight:800,color:'#0f172a'}}>{cust.name} <span style={{fontSize:14,color:'#64748b'}}>({cust.alpha_tag})</span></div>
            <div style={{fontSize:13,color:'#64748b'}}>Tier {cust.adidas_ua_tier} | {o.default_markup||1.65}x catalog | Tax: {cust.tax_rate?(cust.tax_rate*100).toFixed(2)+'%':'N/A'}</div></div>}
          {isSO&&o.estimate_id&&<div style={{fontSize:11,color:'#7c3aed'}}>🔗 From: {o.estimate_id}</div>}
          <div style={{fontSize:11,color:'#94a3b8',marginTop:2}}>Created by {REPS.find(r=>r.id===o.created_by)?.name} · {o.created_at}</div>
        </div>
        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          {[{l:'REVENUE',v:totals.rev,bg:'#f0fdf4',c:'#166534'},{l:'COST',v:totals.cost,bg:'#fef2f2',c:'#dc2626'},{l:'MARGIN',v:totals.margin,bg:'#dbeafe',c:'#1e40af',sub:`${totals.pct.toFixed(1)}%`},{l:'TOTAL',v:totals.grand,bg:'#faf5ff',c:'#7c3aed',sub:'+tax+ship'}].map(x=>
            <div key={x.l} style={{textAlign:'center',padding:'8px 14px',background:x.bg,borderRadius:8,minWidth:75}}><div style={{fontSize:9,color:x.c,fontWeight:700}}>{x.l}</div><div style={{fontSize:18,fontWeight:800,color:x.c}}>${x.v.toLocaleString(undefined,{maximumFractionDigits:0})}</div>{x.sub&&<div style={{fontSize:9,color:'#94a3b8'}}>{x.sub}</div>}</div>)}
        </div>
      </div>
      {/* CONTROLS ROW */}
      <div style={{display:'flex',gap:8,marginTop:12,alignItems:'end',flexWrap:'wrap'}}>
        <div style={{flex:1,minWidth:180}}><label className="form-label">Memo</label><input className="form-input" value={o.memo} onChange={e=>sv('memo',e.target.value)} placeholder="Order description..." style={{fontSize:14}}/></div>
        {!isSO&&<div style={{width:70}}><label className="form-label">Markup</label><input className="form-input" type="number" step="0.05" value={o.default_markup} onChange={e=>{const m=parseFloat(e.target.value)||1.65;sv('default_markup',m);sv('items',o.items.map(it=>isAU(it.brand)?it:{...it,unit_sell:rQ(it.nsa_cost*m)}))}}/></div>}
        {isSO&&<div style={{width:120}}><label className="form-label">Expected Date</label><input className="form-input" type="date" value={o.expected_date||''} onChange={e=>sv('expected_date',e.target.value)}/></div>}
        <button className="btn btn-primary" onClick={()=>{onSave(o);setSaved(true);onNotify(`${isEst?'Estimate':'SO'} saved`)}}><Icon name="check" size={14}/> Save</button>
        {isEst&&saved&&o.status!=='approved'&&<button className="btn btn-secondary" onClick={()=>{sv('status','sent');sv('email_status','sent');onSave({...o,status:'sent',email_status:'sent'});onNotify('Sent to coach')}}><Icon name="send" size={14}/> Send</button>}
        {isEst&&o.status==='approved'&&<button className="btn btn-primary" style={{background:'#7c3aed'}} onClick={()=>onConvertSO(o)}><Icon name="box" size={14}/> Convert to SO</button>}
      </div>
      {/* SHIPPING */}
      <div style={{display:'flex',gap:12,marginTop:12,alignItems:'end',flexWrap:'wrap',borderTop:'1px solid #f1f5f9',paddingTop:12}}>
        <div><label className="form-label">Shipping</label><div style={{display:'flex',gap:4,alignItems:'center'}}>
          ...
          {o.shipping_type==='pct'?<span style={{display:'inline-flex',alignItems:'center',border:'1px solid #d1d5db',borderRadius:4,padding:'2px 6px',background:'white'}}><input value={o.shipping_value||0} onChange={e=>sv('shipping_value',parseFloat(e.target.value)||0)} style={{width:40,border:'none',outline:'none',fontSize:15,fontWeight:800,textAlign:'center',background:'transparent'}}/><span style={{fontWeight:700,color:'#475569'}}>%</span></span>
        ...
        <div style={{flex:1,minWidth:200}}><label className="form-label">Ship To</label>
          {!showNewAddr?<div style={{display:'flex',gap:4}}><select className="form-select" value={o.ship_to_id||'default'} onChange={e=>{if(e.target.value==='new'){setShowNewAddr(true)}else{sv('ship_to_id',e.target.value)}}}>
            {addrs.map(a=><option key={a.id} value={a.id}>{a.label}</option>)}
            <option value="new">+ New Address</option></select></div>
          :<div style={{display:'flex',gap:4}}><input className="form-input" placeholder="Enter new address..." value={newAddr} onChange={e=>setNewAddr(e.target.value)} autoFocus style={{flex:1}}/>
            <button className="btn btn-sm btn-primary" onClick={()=>{sv('ship_to_custom',newAddr);sv('ship_to_id','custom');setShowNewAddr(false)}}>Save</button>
            <button className="btn btn-sm btn-secondary" onClick={()=>setShowNewAddr(false)}>Cancel</button></div>}
          {o.ship_to_id==='custom'&&o.ship_to_custom&&<div style={{fontSize:11,color:'#7c3aed',marginTop:2}}>Custom: {o.ship_to_custom}</div>}
        </div>
      ...
      {/* SO STATUS PROGRESSION */}
      {isSO&&<div style={{display:'flex',gap:4,marginTop:12,borderTop:'1px solid #f1f5f9',paddingTop:12,flexWrap:'wrap'}}>
        {statusFlow.map((sf,i)=>{const sc=STATUS_COLORS[sf]||{};const active=i<=si;const current=i===si;
          return<button key={sf} className="btn btn-sm" style={{background:active?sc.bg:'#f8fafc',color:active?sc.color:'#94a3b8',border:current?`2px solid ${sc.color}`:'1px solid #e2e8f0',fontWeight:current?800:500}} disabled={i<si}
            onClick={()=>{sv('status',sf);onNotify(`Status → ${sf.replace(/_/g,' ')}`)}}>{sf.replace(/_/g,' ')}</button>})}
      </div>}
      {isSO&&<div style={{marginTop:8}}><label className="form-label">Production Notes</label><input className="form-input" value={o.production_notes||''} onChange={e=>sv('production_notes',e.target.value)} placeholder="Internal notes for production team..."/></div>}
    </div></div>

    {/* TABS for SO */}
    {isSO&&<div className="tabs" style={{marginBottom:16}}>{[['items','Line Items & Decorations'],['art','Art Library ('+artFiles.length+')'],['transactions','Linked Transactions'],['firm_dates','Firm Dates ('+((o.firm_dates||[]).length)+')']].map(([k,l])=>
      <button key={k} className={`tab ${tab===k?'active':''}`} onClick={()=>setTab(k)}>{l}</button>)}</div>}

    {/* LINE ITEMS TAB (or always visible for estimates) */}
    {(isEst||tab==='items')&&<>{o.items.map((item,idx)=>{const qty=Object.values(item.sizes).reduce((a,v)=>a+v,0);
      ...
      return(<div key={idx} className="card" style={{marginBottom:12}}>
          ...
          <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
            <div style={{flex:1}}>
              <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}><span style={{fontFamily:'monospace',fontWeight:800,color:'#1e40af',background:'#dbeafe',padding:'3px 10px',borderRadius:4,fontSize:15}}>{item.sku}</span>
                ...
                {item._colors?<select className="form-select" style={{fontSize:12,width:150}} value={item.color||item._colors[0]} onChange={e=>uI(idx,'color',e.target.value)}>{item._colors.map(c=><option key={c} value={c}>{c}</option>)}</select>:<span className="badge badge-gray">{item.color}</span>}</div>
        ...
        <div style={{padding:'8px 18px 14px'}}>
          {item.decorations.map((deco,di)=>{const dp=dP(deco,qty);const artFile=artFiles.find(f=>f.id===deco.art_file_id);return(<div key={di} style={{padding:'10px 0',borderTop:di>0?'1px solid #f1f5f9':''}}>
            <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',marginBottom:6}}>
              ...
              {o.items.length>1&&<select className="form-select" style={{width:150,fontSize:11}} value={deco.art_group||''} onChange={e=>uD(idx,di,'art_group',e.target.value||null)}>
                <option value="">Unique art</option>{o.items.map((oi,oj)=>oj!==idx?<option key={oj} value={`shared-${oj}`}>Same as {oi.sku}</option>:null)}</select>}
              {isSO&&<select className="form-select" style={{width:160,fontSize:11,border:!deco.art_file_id?'2px solid #f59e0b':'1px solid #d1d5db'}} value={deco.art_file_id||''} onChange={e=>uD(idx,di,'art_file_id',e.target.value||null)}>
                <option value="">⚠️ No art assigned</option>{artFiles.map(f=><option key={f.id} value={f.id}>{f.name} ({f.status})</option>)}</select>}
              {isSO&&artFile&&<span style={{fontSize:10,padding:'2px 6px',borderRadius:4,background:artFile.status==='approved'?'#dcfce7':'#fef3c7',color:artFile.status==='approved'?'#166534':'#92400e'}}>📎 {artFile.status}</span>}
            ...
            {deco.art_group&&<div style={{fontSize:10,color:'#7c3aed',marginTop:4}}>🔗 Shared art — one setup</div>}
          </div>)})}
    ...
    <div className="card"><div style={{padding:'14px 18px'}}>
      ...
      :<div><div className="search-bar" style={{marginBottom:8}}><Icon name="search"/><input placeholder="Search SKU, name, brand..." value={pSearch} onChange={e=>setPSearch(e.target.value)} autoFocus/></div>
    ...
    {/* ART LIBRARY TAB (SO only) */}
    {isSO&&tab==='art'&&<div className="card"><div className="card-header"><h2>Art Library</h2><button className="btn btn-sm btn-primary" onClick={addArt}><Icon name="upload" size={12}/> Upload Art File</button></div>
      <div className="card-body">{artFiles.length===0?<div className="empty">No art files uploaded. Upload files to assign to decorations.</div>:
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {artFiles.map((af,i)=><div key={af.id} style={{display:'flex',gap:12,alignItems:'flex-start',padding:12,background:'#f8fafc',borderRadius:8,border:af.status==='approved'?'2px solid #22c55e':'1px solid #e2e8f0'}}>
            <div style={{width:48,height:48,background:'#dbeafe',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><Icon name="file" size={24}/></div>
            <div style={{flex:1}}>
              <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:4}}>
                <input className="form-input" value={af.name} onChange={e=>uArt(i,'name',e.target.value)} style={{fontWeight:700,fontSize:13,flex:1}}/>
                <span style={{padding:'2px 8px',borderRadius:10,fontSize:11,fontWeight:600,background:af.status==='approved'?'#dcfce7':af.status==='uploaded'?'#fef3c7':'#f1f5f9',color:af.status==='approved'?'#166534':af.status==='uploaded'?'#92400e':'#64748b'}}>{af.status}</span>
              </div>
              <input className="form-input" value={af.notes||''} onChange={e=>uArt(i,'notes',e.target.value)} placeholder="Notes about this file..." style={{fontSize:12,marginBottom:4}}/>
              <div style={{display:'flex',gap:4,alignItems:'center'}}>
                <span style={{fontSize:10,color:'#94a3b8'}}>Uploaded {af.uploaded}</span>
                <span style={{fontSize:10,color:'#94a3b8'}}>·</span>
                <span style={{fontSize:10,color:'#64748b'}}>Used in: {o.items.reduce((a,it)=>a+it.decorations.filter(d=>d.art_file_id===af.id).length,0)} decoration(s)</span>
              </div>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:4}}>
              {af.status!=='approved'&&<button className="btn btn-sm btn-primary" style={{fontSize:10}} onClick={()=>{uArt(i,'status','approved');onNotify('Art approved')}}>✓ Approve</button>}
              {af.status==='approved'&&<button className="btn btn-sm btn-secondary" style={{fontSize:10}} onClick={()=>uArt(i,'status','uploaded')}>Unapprove</button>}
              <button className="btn btn-sm btn-secondary" style={{fontSize:10}} onClick={()=>rmArt(i)}><Icon name="trash" size={10}/></button>
            </div>
          </div>)}
        </div>}
        {artFiles.length>0&&!allArtAssigned&&<div style={{marginTop:12,padding:10,background:'#fef3c7',borderRadius:6,fontSize:12,color:'#92400e'}}>⚠️ Some decorations don't have art assigned. Go to Line Items tab to assign art files.</div>}
      </div></div>}

    {/* LINKED TRANSACTIONS TAB (SO only) */}
    {isSO&&tab==='transactions'&&<div className="card"><div className="card-header"><h2>Linked Transactions</h2></div><div className="card-body">
      <div style={{display:'flex',flexDirection:'column',gap:8}}>
        {o.estimate_id&&<div style={{display:'flex',gap:12,alignItems:'center',padding:12,background:'#faf5ff',borderRadius:8,border:'1px solid #e9d5ff'}}>
          ...
          <div><div style={{fontWeight:700,color:'#7c3aed'}}>{o.estimate_id}</div><div style={{fontSize:12,color:'#64748b'}}>Source Estimate</div></div>
          <span className="badge badge-green">Converted</span></div>}
        <div style={{padding:12,background:'#f8fafc',borderRadius:8,border:'1px solid #e2e8f0'}}>
          <div style={{fontWeight:600,marginBottom:4}}>Purchase Orders</div>
          <div style={{fontSize:12,color:'#94a3b8'}}>No POs linked yet (Phase 4)</div></div>
        <div style={{padding:12,background:'#f8fafc',borderRadius:8,border:'1px solid #e2e8f0'}}>
          <div style={{fontWeight:600,marginBottom:4}}>Invoices</div>
          <div style={{fontSize:12,color:'#94a3b8'}}>No invoices linked yet</div></div>
      </div>
    </div></div>}

    {/* FIRM DATES TAB (SO only) */}
    {isSO&&tab==='firm_dates'&&<div className="card"><div className="card-header"><h2>Firm Date Requests</h2><button className="btn btn-sm btn-primary" onClick={()=>{sv('firm_dates',[...(o.firm_dates||[]),{item_desc:'',date:'',approved:false}])}}><Icon name="plus" size={12}/> Add Request</button></div>
      <div className="card-body">{(o.firm_dates||[]).length===0?<div className="empty">No firm date requests</div>:
        <table><thead><tr><th>Item Description</th><th>Requested Date</th><th>Status</th><th>Action</th></tr></thead><tbody>
        {(o.firm_dates||[]).map((fd,i)=><tr key={i}>
          <td><input className="form-input" value={fd.item_desc} onChange={e=>{const fds=[...(o.firm_dates||[])];fds[i]={...fds[i],item_desc:e.target.value};sv('firm_dates',fds)}} placeholder="e.g. Game Jerseys"/></td>
          ...
          <td><div style={{display:'flex',gap:4}}>{!fd.approved&&<button className="btn btn-sm btn-primary" onClick={()=>{const fds=[...(o.firm_dates||[])];fds[i]={...fds[i],approved:true};sv('firm_dates',fds);onNotify('Approved')}}>Approve</button>}
            <button className="btn btn-sm btn-secondary" onClick={()=>sv('firm_dates',(o.firm_dates||[]).filter((_,x)=>x!==i))}><Icon name="trash" size={10}/></button></div></td>
...
function CustDetail({customer,allCustomers,allOrders,onBack,onEdit,onSelCust,onNewEst}){
  ...
  const tl={prepay:'Prepay',net15:'Net 15',net30:'Net 30',net60:'Net 60'};const tierL={A:'40% off',B:'35% off',C:'30% off'};
  ...
  {subs.map(sub=><div key={sub.id} style={{padding:'10px 18px',borderBottom:'1px solid #f1f5f9',display:'flex',alignItems:'center',gap:8,cursor:'pointer'}} onClick={()=>onSelCust(sub)}>
    ...
    {(sub._open_balance||0)>0&&<span style={{fontSize:12,fontWeight:700,color:'#dc2626'}}>${sub._open_balance.toLocaleString()}</span>}</div>)}</div></div>}
  ...
  {tab==='overview'&&<div className="card"><div className="card-header"><h2>Customer Info</h2></div><div className="card-body">
    ...
    <div style={{marginTop:16}}><div className="form-label">Pricing</div><div style={{fontSize:13}}>Tier {customer.adidas_ua_tier} ({tierL[customer.adidas_ua_tier]}) | Catalog: {customer.catalog_markup||1.65}x</div></div>
  ...
  {tab==='artwork'&&<div className="card"><div className="card-body"><div className="empty">Customer art library (aggregates from all SOs) — Phase 3</div></div></div>}
  {tab==='reporting'&&<div className="card"><div className="card-header"><h2>Reporting</h2><div style={{display:'flex',gap:4}}>{[['thisyear','This Year'],['lastyear','Last Year'],['rolling','Rolling 12'],['alltime','All']].map(([v,l])=><button key={v} className={`btn btn-sm ${rR===v?'btn-primary':'btn-secondary'}`} onClick={()=>setRR(v)}>{l}</button>)}</div></div>
    <div className="card-body"><div className="stats-row"><div className="stat-card"><div className="stat-label">Revenue</div><div className="stat-value">{rR==='thisyear'?'$15,600':'$32,600'}</div></div><div className="stat-card"><div className="stat-label">Orders</div><div className="stat-value">{rR==='thisyear'?'4':'8'}</div></div><div className="stat-card"><div className="stat-label">Avg Order</div><div className="stat-value">{rR==='thisyear'?'$3,900':'$4,075'}</div></div><div className="stat-card"><div className="stat-label">LTV</div><div className="stat-value">$48,200</div></div></div></div></div>}
  </div>);
}
...
export default function App(){
  ...
  const rDash=()=>(<>
    ...
    {isA&&al.length>0&&<div className="card" style={{marginBottom:16,borderLeft:'4px solid #d97706'}}><div className="card-header"><h2 style={{color:'#d97706'}}><Icon name="alert" size={16}/> Stock Alerts</h2></div>
    ...
    {ests.slice(0,5).map(e=>{const c=cust.find(x=>x.id===e.customer_id);return(<tr key={e.id} style={{cursor:'pointer'}} onClick={()=>{setEEst(e);setEEstC(c);setPg('estimates')}}>
      ...
      <td><EmailBadge est={e}/></td></tr>)})}
    </tbody></table></div></div></>);

  // ESTIMATES
  const rEst=()=>{
    if(eEst)return<OrderEditor order={eEst} mode="estimate" customer={eEstC} allCustomers={cust} products={prod} onSave={e=>{savE(e);setEEst(e)}} onBack={()=>setEEst(null)} onConvertSO={convertSO} currentUser={cu} onNotify={nf}/>;
    ...
    return(<><div style={{display:'flex',gap:8,marginBottom:16}}><div className="search-bar" style={{flex:1}}><Icon name="search"/><input placeholder="Search ID, customer, memo..." value={q} onChange={e=>setQ(e.target.value)}/></div>
      ...
      <div className="card"><div className="card-body" style={{padding:0}}><table><thead><tr><th>ID</th><th>Customer</th><th>Memo</th><th>Items</th><th>Status</th><th>Email</th><th>Actions</th></tr></thead><tbody>
  ...
  // SALES ORDERS
  const rSO=()=>{
    if(eSO)return<OrderEditor order={eSO} mode="so" customer={eSOC} allCustomers={cust} products={prod} onSave={s=>{savSO(s);setESO(s)}} onBack={()=>setESO(null)} currentUser={cu} onNotify={nf}/>;
    ...
    <div className="card"><div className="card-body" style={{padding:0}}><table><thead><tr><th>SO</th><th>Customer</th><th>Memo</th><th>Expected</th><th>Art</th><th>Status</th><th>Created</th></tr></thead><tbody>
    {sos.map(so=>{const c=cust.find(x=>x.id===so.customer_id);const artCount=(so.art_files||[]).length;const artApproved=(so.art_files||[]).filter(f=>f.status==='approved').length;
      ...
      <td>{artCount>0?<span style={{fontSize:11}}>{artApproved}/{artCount} ✓</span>:<span style={{fontSize:11,color:'#d97706'}}>None</span>}</td>
      <td><span style={{padding:'3px 10px',borderRadius:12,fontSize:11,fontWeight:700,background:STATUS_COLORS[so.status]?.bg||'#f1f5f9',color:STATUS_COLORS[so.status]?.color||'#475569'}}>{so.status.replace(/_/g,' ')}</span></td>
  ...
  const rCust=()=>{
    ...
    {f.map(p=>{const kids=gK(p.id);const bal=kids.reduce((a,c)=>a+(c._open_balance||0),p._open_balance||0);const est=kids.reduce((a,c)=>a+(c._open_estimates||0),p._open_estimates||0);
      return(<div key={p.id} className="card" style={{marginBottom:10}}>
        <div style={{padding:'12px 16px',display:'flex',alignItems:'center',gap:12}}>
          ...
          <div style={{flex:1,cursor:'pointer'}} onClick={()=>setSelC(p)}>
            <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}><span style={{fontSize:15,fontWeight:700}}>{p.name}</span><span className="badge badge-blue">{p.alpha_tag}</span><span className="badge badge-green">Tier {p.adidas_ua_tier}</span>
              {est>0&&<span className="badge badge-amber">{est} est</span>}</div>
        ...
        {kids.length>0&&<div style={{borderTop:'1px solid #f1f5f9'}}>{kids.map(ch=><div key={ch.id} style={{padding:'8px 16px 8px 64px',display:'flex',alignItems:'center',gap:10,borderBottom:'1px solid #f8fafc',cursor:'pointer'}} onClick={()=>setSelC(ch)}>
          ...
          {(ch._open_balance||0)>0&&<span style={{fontSize:12,fontWeight:700,color:'#dc2626'}}>${ch._open_balance.toLocaleString()}</span>}</div>)}</div>}
      </div>)})}
    </>);
  };
  ...
  const rVend=()=>{if(selV)return<VendDetail vendor={selV} onBack={()=>setSelV(null)}/>;
    return(<><div className="stats-row"><div className="stat-card"><div className="stat-label">Vendors</div><div className="stat-value">{vend.length}</div></div><div className="stat-card"><div className="stat-label">API</div><div className="stat-value">{vend.filter(v=>v.vendor_type==='api').length}</div></div>
      {isA&&<div className="stat-card"><div className="stat-label">Open AP</div><div className="stat-value" style={{color:'#dc2626'}}>${vend.reduce((a,v)=>a+(v._invoice_total||0),0).toLocaleString()}</div></div>}</div>
    <div className="card"><div className="card-body" style={{padding:0}}><table><thead><tr><th>Vendor</th><th>Type</th><th>Contact</th><th>Terms</th>{isA&&<th>Owed</th>}{isA&&<th>Aging</th>}<th>Status</th></tr></thead><tbody>
    {vend.map(v=><tr key={v.id} style={{cursor:'pointer'}} onClick={()=>setSelV(v)}>
      ...
      {isA&&<td style={{fontSize:11}}>{(v._aging_30||0)>0&&<span style={{color:'#d97706'}}>30d:${v._aging_30.toLocaleString()} </span>}{(v._aging_60||0)>0&&<span style={{color:'#dc2626'}}>60d:${v._aging_60.toLocaleString()}</span>}</td>}
  ...
  const rProd=()=>(<><div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
    ...
    <select className="form-select" style={{width:120}} value={pF.cat} onChange={e=>setPF(f=>({...f,cat:e.target.value}))}><option value="all">Category</option>{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select>
    ...
    <select className="form-select" style={{width:140}} value={pF.clr} onChange={e=>setPF(f=>({...f,clr:e.target.value}))}><option value="all">Color</option>{cols.map(c=><option key={c}>{c}</option>)}</select></div>
  ...
  {fP.map(p=>{const nsaT=Object.values(p._inv||{}).reduce((a,v)=>a+v,0);const au=p.brand==='Adidas'||p.brand==='Under Armour';
    return(<div key={p.id} style={{padding:'14px 16px',borderBottom:'1px solid #f1f5f9'}}><div style={{display:'flex',gap:14,alignItems:'flex-start'}}>
      ...
      <div style={{flex:1}}>
        ...
        <div style={{fontSize:12,color:'#94a3b8',marginTop:2}}><span className="badge badge-blue" style={{marginRight:6}}>{p.brand}</span>{p.color} | Cost: ${p.nsa_cost?.toFixed(2)} | Sell: {au?'Tier':'$'+rQ(p.nsa_cost*1.65).toFixed(2)}</div>
        <div style={{display:'flex',gap:2,marginTop:8,flexWrap:'wrap',alignItems:'center'}}>
          {p.available_sizes.filter(sz=>showSz(sz,p._inv?.[sz])).map(sz=>{const v=p._inv?.[sz]||0;const a2=p._alerts?.[sz];return<div key={sz} className={`size-cell ${v>10?'in-stock':v>0?(a2&&v<a2?'alert-stock':'low-stock'):'no-stock'}`}><div className="size-label">{sz}</div><div className="size-qty">{v}</div></div>})}
  ...
  const rInv=()=>(<><div className="stats-row"><div className="stat-card"><div className="stat-label">Units</div><div className="stat-value">{tU}</div></div><div className="stat-card"><div className="stat-label">Value</div><div className="stat-value">${tV.toLocaleString(undefined,{maximumFractionDigits:0})}</div></div><div className="stat-card"><div className="stat-label">Products</div><div className="stat-value">{iD.length}</div></div>
    {isA&&<div className="stat-card" style={al.length>0?{borderColor:'#fbbf24'}:{}}><div className="stat-label">Alerts</div><div className="stat-value" style={{color:al.length>0?'#d97706':''}}>{al.length}</div></div>}</div>
  <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap'}}><div className="search-bar" style={{flex:1,minWidth:200}}><Icon name="search"/><input placeholder="Search inventory..." value={q} onChange={e=>setQ(e.target.value)}/></div>
    <select className="form-select" style={{width:120}} value={iF.cat} onChange={e=>setIF(f=>({...f,cat:e.target.value}))}><option value="all">Category</option>{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select>
    <select className="form-select" style={{width:120}} value={iF.vnd} onChange={e=>setIF(f=>({...f,vnd:e.target.value}))}><option value="all">Vendor</option>{vend.map(v=><option key={v.id} value={v.id}>{v.name}</option>)}</select></div>
  ...
  <tbody>{iD.map(p=><tr key={p.id}>
    ...
    <td><div style={{display:'flex',gap:4}}><button className="btn btn-sm btn-secondary" onClick={()=>nf('PO (Phase 4)')}>+PO</button><button className="btn btn-sm btn-secondary" onClick={()=>newE(null)}>+EST</button>
      {isA&&<button className="btn btn-sm btn-secondary" onClick={()=>setAM({open:true,p})}><Icon name="edit" size={10}/> INV</button>}</div></td>
  </tr>)}</tbody></table></div></div></>);

  // NAV & LAYOUT
  const nav=[{section:'Overview'},{id:'dashboard',label:'Dashboard',icon:'home'},{section:'Sales'},{id:'estimates',label:'Estimates',icon:'dollar'},{id:'orders',label:'Sales Orders',icon:'box'},{section:'People'},{id:'customers',label:'Customers',icon:'users'},{id:'vendors',label:'Vendors',icon:'building'},{section:'Catalog'},{id:'products',label:'Products',icon:'package'},{id:'inventory',label:'Inventory',icon:'warehouse'},{section:'Coming'},{id:'production',label:'Production',icon:'grid',disabled:true},{id:'artwork',label:'Artwork',icon:'image',disabled:true}];
  ...
  return(<div className="app"><Toast message={toast?.msg} type={toast?.type}/>
    <div className="sidebar"><div className="sidebar-logo">NSA<span>Operations Portal</span></div>
      <nav className="sidebar-nav">{nav.map((item,i)=>{if(item.section)return<div key={i} className="sidebar-section">{item.section}</div>;
        return<button key={item.id} className={`sidebar-link ${pg===item.id?'active':''}`} disabled={item.disabled} style={item.disabled?{opacity:0.3,cursor:'not-allowed'}:{}}
          onClick={()=>{if(!item.disabled){setPg(item.id);setQ('');setSelC(null);setSelV(null);setEEst(null);setESO(null)}}}><Icon name={item.icon}/>{item.label}</button>})}</nav>
      <div className="sidebar-user"><div style={{fontWeight:600,color:'#e2e8f0'}}>{cu.name}</div><div>{cu.role}</div></div></div>
    <div className="main"><div className="topbar"><h1>{eEst?eEst.id:eSO?eSO.id:selC?selC.name:selV?selV.name:(titles[pg]||'Dashboard')}</h1><div style={{fontSize:12,color:'#94a3b8'}}>Phase 2 v4</div></div>
      <div className="content">{pg==='dashboard'&&rDash()}{pg==='estimates'&&rEst()}{pg==='orders'&&rSO()}{pg==='customers'&&rCust()}{pg==='vendors'&&rVend()}{pg==='products'&&rProd()}{pg==='inventory'&&rInv()}</div></div>
