...
export async function renderProductionSVG(input, opts = {}) {
  ...
  // Keep any artist-supplied vector master inside the production deliverable.
  // The live garment uses its UV-aligned atlas, while this hidden named layer
  // preserves the original editable source for the production artist instead
  // of reducing the job to screenshots or a camera projection.
  const vectorMasters = [];
  for (let row = 0; row < entries.length; row += 1) {
    const entry = entries[row];
    const template = getTemplate(entry.spec.garmentId);
    if (!template.productionVector) continue;
    const href = await inlineAsset(template.productionVector);
    vectorMasters.push(`<g id="source-vector-${row + 1}" inkscape:groupmode="layer" inkscape:label="${esc(entry.label)} Artist Vector Master (hidden)" data-source-vector="${esc(template.productionVector)}" style="display:none"><image href="${esc(href)}" x="40" y="${100 + row * ROW_H}" width="1080" height="450" preserveAspectRatio="xMidYMid meet"/></g>`);
  }
  ...
  const metadata = {
    ...
    notes: 'Text remains editable and requires the recorded font. Raster logos/patterns remain embedded at source resolution. Raster-backed garments use independent embedded alpha masks for exact editable color boundaries. Artist vector masters are preserved in hidden named layers when supplied.',
    spec,
