...
function tint(lx, zoneSpec, w, h) {
  ...
  if (pat === 'custom' && zoneSpec.patternImage) {
    ...
    if (img && zoneSpec.patternTint) img = tintedTile(img, zoneSpec.patternImage, color, patternColor2, ds.toHex(zoneSpec.color3, '#ffffff'), ds.toHex(zoneSpec.color4, '#ffffff'), zoneSpec.patternTintMode);
