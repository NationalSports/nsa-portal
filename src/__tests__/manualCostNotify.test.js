...
describe('manual cost email notification', () => {
  ...
  test('emails Steve only after reading back the persisted cost and verified poster', async () => {
    const admin = makeAdmin({
      ...
      sales_orders: { data: { id: 'SO-1863', customer_id: 'cust-1', memo: 'Champ Gear' }, error: null },
      team_members: { data: { id: 'tm-1', name: 'Jered Hunt', email: 'jered@nationalsportsapparel.com' }, error: null },
      customers: { data: { id: 'cust-1', name: 'College of San Mateo Football' }, error: null },
    });
    ...
    expect(sent.textContent).toContain('Posted by: Jered Hunt (jered@nationalsportsapparel.com)');
