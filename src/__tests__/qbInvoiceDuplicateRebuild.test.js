...
function setup({existingLines,rebuildResponse}={}){
  ...
  const qbApi=jest.fn(async(action,args={})=>{
    if(action==='query'){
      ...
      if(q.includes('FROM Invoice')&&q.includes("Id = '1084'"))return{QueryResponse:{Invoice:[{Id:'1084',SyncToken:'2',CustomerRef:{value:'55'},TotalAmt:3232.4,TxnDate:'2026-08-31',TxnTaxDetail:{TotalTax:0},Line:existingLines}]}};
    ...
    if(action==='upsert_invoice'){
      ...
      return{Invoice:{Id:'1084',TotalAmt:3232.4,TxnTaxDetail:{TotalTax:0},Line:args.invoice.Line}};
    }
