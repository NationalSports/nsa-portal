...
function OrderEditor({order,mode,customer:ic,allCustomers,products,vendors:vendorsProp,onSave,onSaveArtFiles,onBack,onConvertSO,onCopyEstimate,onCopySalesOrder,onRevertToEst,onSetJobLinkGroup,onSetJobAutoGroupOff,cu,nf,msgs,onMsg,dirtyRef,onAdjustInv,allOrders,onInv,allInvoices,batchPOs,onBatchPO,nextBatchPONumber,initTab,onNavCustomer,onNewEstimate,scrollToItem,scrollToJob,scrollToJobRef,onScrollJobConsumed,openPOId,onOpenPOConsumed,reps:REPS,ssConnected,ssShipping,onShipSS,onCheckShipStatus,onDelete,onNavInvoice,onNavBatch,onSaveProduct,onViewEstimate,onViewSO,returnToPage,onReturnToJob,onAssignTodo,assignedTodos,onCompleteTodo,portalSettings,decoVendors:decoVendorsProp,decoVendorPricing:decoVendorPricingProp,changeLog:changeLogProp,dbSavePromoPeriod:_dbSavePromoPeriod,onSavePromoPeriod,onSavePromoUsage,onDeletePromoUsage,companyInfo:companyInfoProp,fetchAdidasInventory:fetchAdidasInventoryProp,searchProducts:searchProductsProp,onSaveCustomer,onScheduleEmail,supabase}){
  ...
  // resolveVendor keys purely off the new vendor_id, drop the now-stale vendor inventory,
  ...
  return(<div>
            ...
            <div style={{position:'relative'}}>
              ...
              {showItemMenu===idx&&itemMenuPos&&createPortal(<>
                ...
                <div style={{position:'fixed',...(itemMenuPos.top!=null?{top:itemMenuPos.top}:{bottom:itemMenuPos.bottom}),right:itemMenuPos.right,maxHeight:itemMenuPos.maxH,overflowY:'auto',background:'white',border:'1px solid #e2e8f0',borderRadius:6,boxShadow:'0 8px 24px rgba(0,0,0,0.12)',zIndex:1040,minWidth:200,padding:4}}>
                  ...
                  {item.is_free_promo
                    ?<button onClick={()=>{sv('items',safeItems(o).map((it,xi)=>xi===idx?{...it,is_free_promo:false,unit_sell:it._pre_free_promo_sell!=null?it._pre_free_promo_sell:it.unit_sell,_pre_free_promo_sell:undefined,nsa_cost:it._pre_free_promo_cost!=null?it._pre_free_promo_cost:it.nsa_cost,_pre_free_promo_cost:undefined}:it));setShowItemMenu(null);setItemMenuPos(null);nf('Promo item removed — pricing restored')}} style={{display:'flex',alignItems:'center',gap:8,width:'100%',padding:'6px 10px',background:'none',border:'none',cursor:'pointer',color:'#be185d',fontSize:12,fontWeight:600,textAlign:'left',borderRadius:4}} onMouseEnter={e=>e.currentTarget.style.background='#fdf2f8'} onMouseLeave={e=>e.currentTarget.style.background='none'}><span style={{display:'inline-block',width:14,textAlign:'center',fontSize:12}}>🎁</span> Remove Promo Item</button>
                    :<button onClick={()=>{sv('items',safeItems(o).map((it,xi)=>xi===idx?{...it,is_free_promo:true,_pre_free_promo_sell:it.unit_sell,unit_sell:0,_pre_free_promo_cost:it.nsa_cost,nsa_cost:0}:it));setShowItemMenu(null);setItemMenuPos(null);nf('Item marked as free promo — $0 garment, deco charges still apply')}} style={{display:'flex',alignItems:'center',gap:8,width:'100%',padding:'6px 10px',background:'none',border:'none',cursor:'pointer',color:'#be185d',fontSize:12,fontWeight:600,textAlign:'left',borderRadius:4}} onMouseEnter={e=>e.currentTarget.style.background='#fdf2f8'} onMouseLeave={e=>e.currentTarget.style.background='none'}><span style={{display:'inline-block',width:14,textAlign:'center',fontSize:12}}>🎁</span> Mark as Promo Item</button>}
                  <div style={{height:1,background:'#e2e8f0',margin:'4px 0'}}/>
    ...
    {aiBuild&&<div className="modal-overlay" onClick={()=>!aiBuild.loading&&setAiBuild(null)}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:900,maxHeight:'90vh',overflow:'auto'}}>
      ...
      {aiBuild.step==='review'&&<>
        ...
        <div style={{maxHeight:380,overflow:'auto',border:'1px solid #e2e8f0',borderRadius:6}}>
          <table style={{fontSize:11}}><thead><tr><th style={{width:30}}>✓</th><th>SKU</th><th>Match</th><th>Attach item</th><th>Name</th><th>Brand</th><th>Color</th><th>Sizes</th><th>Qty</th><th>Notes</th></tr></thead>
          <tbody>{aiBuild.parsed.map((it,i)=>{
            ...
            const bindOrder=p=>setAiBuild(x=>({...x,parsed:x.parsed.map((q,qi)=>qi===i?{...q,product_id:p.id,sku_guess:p.sku,name:p.name||q.name,brand:p.brand||q.brand,color:p.color||q.color,match_quality:'manual',vendor_source:null}:q)}));
      ...
      {typeof showPick!=='object'?<div className="modal-body">
        ...
        {safeItems(o).map((item,idx)=>{const q=Object.values(item.sizes).reduce((a,v)=>a+v,0);const szList=Object.entries(item.sizes).filter(([,v])=>v>0).sort((a,b)=>{const ord=SZ_ORD;return(ord.indexOf(a[0])===-1?99:ord.indexOf(a[0]))-(ord.indexOf(b[0])===-1?99:ord.indexOf(b[0]))});
          ...
          return<div key={idx} style={{padding:10,border:pickSel[idx]?'2px solid #3b82f6':'1px solid #e2e8f0',borderRadius:6,marginBottom:6,cursor:'pointer',display:'flex',alignItems:'center',gap:10,background:pickSel[idx]?'#eff6ff':'white'}} onClick={()=>setPickSel(prev=>({...prev,[idx]:!prev[idx]}))}>
            ...
            <div style={{fontSize:11,color:'#64748b',marginTop:2}}>{szList.map(([sz,v])=>{const inv=availInv(p,sz);const picked=(item.pick_lines||[]).reduce((a,pk)=>a+(pk[sz]||0),0);const po=poCommitted(item.po_lines,sz);const open=Math.max(0,v-picked-po);return open>0?sz+': '+open+' open ('+inv+' inv) ':'';}).filter(Boolean).join(' | ')}</div></div></div>})}</>})()}
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:12}}>
          ...
          <button className="btn btn-primary" disabled={!Object.values(pickSel).some(Boolean)} style={{padding:'8px 20px',fontWeight:700}} onClick={()=>{
            const pickItems=safeItems(o).map((it,i)=>{if(!pickSel[i])return null;const szs2=Object.entries(it.sizes).filter(([,v])=>v>0).sort((a,b)=>(SZ_ORD.indexOf(a[0])===-1?99:SZ_ORD.indexOf(a[0]))-(SZ_ORD.indexOf(b[0])===-1?99:SZ_ORD.indexOf(b[0])));
              ...
              return{...it,_idx:i,_pick:Object.fromEntries(szs2.map(([sz,v])=>{const inv=availInv(pp,sz);const picked=safePicks(it).reduce((a,pk)=>a+(pk[sz]||0),0);const po=poCommitted(it.po_lines,sz);const open=Math.max(0,v-picked-po);return[sz,inv>0?Math.min(open,inv):0]}))}}).filter(Boolean);
    ...
    {poFullPage&&(()=>{
      ...
      if(poFullPage.decoPo){
        ...
        const _makeDecoPoDocOpts=()=>({
          title:dp.vendor||'Decorator',docNum:dp.po_id,docType:'DECORATION PURCHASE ORDER',
          headerRight:'<div class="ta" style="font-size:18px">Status: '+(dp.status||'waiting').replace(/^./,c=>c.toUpperCase())+'</div><div class="ts">'+(dp.qty||0)+' unit'+((dp.qty||0)!==1?'s':'')+' · Expected: <strong>$'+expected.toFixed(2)+'</strong></div>',
          infoBoxes:[
            {label:'Vendor',value:dp.vendor||'—',sub:dp.deco_type?dp.deco_type.replace(/_/g,' '):undefined},
            {label:'Sales Order',value:soId||o.id,sub:(cust?.name||'')+(o.memo?' — '+o.memo:'')},
          ...
          tables:[
            {title:'Cost Summary',headers:['Units Covered','Unit Cost','Expected','Actual (billed)','Bills'],aligns:['center','right','right','right','center'],
              rows:[{cells:[
                ...
                {value:'$'+safeNum(dp.unit_cost).toFixed(2),style:'text-align:right'},
                {value:'$'+expected.toFixed(2),style:'text-align:right;font-weight:700'},
    ...
    {editBatchPO&&(()=>{
      ...
      {/* Change Vendor Modal — reassign which vendor an item is ordered from */}
      {vendorModal&&(()=>{const it=safeItems(o)[vendorModal.itemIdx];if(!it)return null;
        const curVid=it.vendor_id||'';const curName=vendorList.find(v=>v.id===curVid)?.name;
        const hasPO=safePOs(it).length>0;
        return<div className="modal-overlay" style={{zIndex:10001}} onClick={()=>setVendorModal(null)}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:440}}>
          <div className="modal-header"><h2>Change Vendor</h2><button className="modal-close" onClick={()=>setVendorModal(null)}>×</button></div>
        ...
        return<div className="modal-overlay" style={{zIndex:10001}} onClick={()=>setCopySkuModal(null)}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:560}}>
          ...
          <div className="modal-body">
            ...
            {isClone?<div style={{display:'flex',flexDirection:'column',gap:10}}>
              <div style={{fontSize:12,color:'#64748b'}}>Adds an exact duplicate of this line (same SKU, sizes, and decorations) below it.</div>
              <button className="btn btn-primary" onClick={()=>{copyI(copySkuModal.itemIdx);setCopySkuModal(null)}} style={{alignSelf:'flex-start'}}><Icon name="file" size={14}/> Add duplicate line</button>
            </div>:<>
