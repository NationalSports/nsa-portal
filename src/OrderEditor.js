...
import { sendBrevoEmail, sendBrevoSms, fileUpload, isUrl, fileDisplayName, _isImgUrl, _isPdfUrl, _cloudinaryPdfThumb, _filterDisplayable, openFile, buildDocHtml, printDoc, nextInvId, _brevoKey, queueProductionFile } from './utils';
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{marginBottom:6}}>
                    ...
                    <div style={{border:'2px dashed #fde68a',borderRadius:6,padding:12,textAlign:'center',cursor:'pointer',background:'#fffbeb',transition:'all 0.15s'}}
                      onClick={()=>{const inp=document.createElement('input');inp.type='file';inp.accept='.pdf,.ai,.eps,.dst,.png,.jpg,.jpeg';inp.multiple=true;inp.onchange=async()=>{let accumulated=[...(art.prod_files||[])];for(const f of inp.files){nf('Uploading '+f.name+'...');try{const url=await fileUpload(f,'nsa-production');accumulated=[...accumulated,{url,name:f.name}];uArt(i,'prod_files',accumulated);nf('✅ '+f.name+' uploaded');queueProductionFile(_sbClient,{so_id:o.id,art_id:art.id,art_name:art.name,deco_type:art.deco_type,file_url:url,file_name:f.name}).then(r=>{if(r?.ok&&!r.skipped)nf('📡 Queued for hot folder: '+f.name)}).catch(()=>{})}catch(e){nf('Upload failed: '+e.message,'error')}}};inp.click()}}
