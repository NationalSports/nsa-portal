-- Expose applied logo art on the public storefront.
--
-- Appends webstore_products.decorations to the storefront view so the public
-- (anon) storefront can render the on-garment logo mock. CREATE OR REPLACE only
-- allows appending columns, so decorations goes last; every other column is
