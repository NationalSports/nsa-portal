-- Expose the scraped product description on the storefront view.
--
-- products.description holds vendor-scraped copy (features, fabric, fit) for most
-- items. The storefront product page can show it, so this appends it to the view.
-- Fully additive (CREATE OR REPLACE only appends the new column, after
-- vendor_size_eta); existing columns are unchanged. Display-side cleanup of empty
-- "LABEL: N/A" spec fields (common in the Adidas feed) happens in the frontend.
--
