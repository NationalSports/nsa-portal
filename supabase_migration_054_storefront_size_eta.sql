-- Per-size restock ETA, so the storefront can show sizes that are "available
-- within ~2 weeks", not just in stock right now.
--
-- The store should only surface a size a shopper can actually get soon: in stock
-- now, OR restocking shortly. We already publish per-size vendor stock
-- (vendor_size_stock); this adds the per-size vendor delivery date for the sizes
-- that are currently out, so the frontend can include a size when its ETA is
-- within ~2 weeks and hide it when the next delivery is months out (e.g. an
-- Adidas style whose 3XL–6XL only return next season).
--
--   • vendor_size_eta — {size: 'YYYY-MM-DD'} from adidas_inventory, only for
--     sizes that are out of stock and carry a valid future delivery date,
--     restricted to the product's available_sizes (same filter as
--     vendor_size_stock). NULL when nothing out-of-stock has a date.
--
-- Fully additive (CREATE OR REPLACE only appends the new column, after
-- store_category). Existing columns are unchanged.
--
 ...
 SELECT wp.id AS webstore_product_id,
   ...
   FROM webstore_products wp
     ...
     LEFT JOIN LATERAL ( SELECT jsonb_object_agg(ai.size, ai.stock_qty) AS vendor_size_stock,
            ...
            jsonb_object_agg(ai.size, ai.future_delivery_date) FILTER (WHERE COALESCE(ai.stock_qty, 0) <= 0 AND ai.future_delivery_date ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$') AS vendor_size_eta
