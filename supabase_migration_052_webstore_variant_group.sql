-- Multiple color options per catalog item (variant grouping).
--
-- Each garment color is its own product (its own SKU, image, sizes, stock), so a
-- store item was one webstore_products row per color — adding a color created a
-- whole new card. This groups colors of the same garment under one card:
--   • adds webstore_products.variant_group_id (text). All colors of one garment
--     share a group id (the primary row's id). NULL = a standalone single-color
--     item (the existing behavior).
--   • exposes variant_group_id on the webstore_storefront_products view so the
--     storefront can group color rows into one product with a color picker.
--
-- Pricing/stock/checkout are unchanged: each color stays a real, individually
-- priced row, so per-color stock, the order line, and POs keep working per SKU.
-- The grouping is a display concern in the builder and storefront. Fully additive
-- (CREATE OR REPLACE only appends the new column, after size_upcharges).
--
...
ALTER TABLE webstore_products ADD COLUMN IF NOT EXISTS variant_group_id text;
