...
function payableReviewStore(admin,realm){return{
  ...
  async snapshot(){
    ...
    const [ledger,portalVendors,products,salesOrders,soItems,poLines,receipts,configResult]=await Promise.all([
      ...
      page((from,to)=>admin.from('sales_orders').select('*').is('deleted_at',null).order('id',{ascending:true}).range(from,to),SNAPSHOT_LIMITS.portalRows),
...
module.exports={payableReviewStore,prefixForRealm,parseValue,SNAPSHOT_LIMITS};
