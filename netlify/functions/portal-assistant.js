...
function buildSystemPrompt({ screen, screens, tours, targets }) {
  ...
  return [
    ...
    '- For ANY "how do I …" / "how do you …" / "walk me through …" / "show me how to …" / "steps to …" procedural question, you MUST call the `guide` tool and build a multi-step walkthrough (at least 2 steps). This is not optional and it replaces a text explanation. Do NOT answer a procedure with a single `highlight`, and do NOT tell the user to "ask a teammate/manager" or that you "don\'t have the steps" for standard navigation — lay out the best step-by-step you can from the screens and targets you were given.',
    '  Compose an ordered list of short, action-oriented steps. For each: write a plain instruction, set `screen` when the step happens on a specific screen (the app moves the user there and the next element appears), and set `target_id` to spotlight an element whenever one from the list below matches. The user advances by clicking the highlighted element, so order the steps as the real click-path: navigate → click → next element → next. Start with the step that gets them to the right screen.',
    '  Ground every step in the screens and targets below — never invent a button, field, tab, or menu you were not given. If a particular step has no matching target, still include it with a clear instruction and omit target_id (it shows as a written step). Keep any step you are less sure about general but actionable (e.g. "add the vendor and the items, then save") rather than fabricating exact labels. Give a one-line lead-in in your text; the steps carry the detail.',
