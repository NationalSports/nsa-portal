...
function buildTools({ tours, targets, screens }) {
  ...
  // enum when the catalog is non-empty (an empty enum is an invalid schema).
  {
    ...
    if (targetEnum.length) stepProps.target_id = { type: 'string', enum: targetEnum, description: 'A KNOWN on-screen element to spotlight, from the provided targets list. Use this when a listed target matches. Omit if none matches.' };
    stepProps.find = { type: 'string', description: "Fallback spotlight for an element NOT in the targets list: its exact on-screen text/label (e.g. \"Settings\", \"Save changes\", \"New Store\", \"Reporting\"). The app locates a visible button/tab/link with that label on the current screen and highlights it. Use the real caption you'd expect on screen. If it can't be found, the step still shows as a written instruction. Prefer target_id when a listed target fits; otherwise use find so you can spotlight almost anything." };
    tools.push({
      name: 'guide',
