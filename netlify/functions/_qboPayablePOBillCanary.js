...
function buildSource({run,row,candidate,realm,qboPurchaseOrderId}){
  ...
  if(clean(raw.po_origin)!=='portal'||paymentMethod||freight!==0||sportsFee!==0||!poNumber||!clean(qboPurchaseOrderId))throw new Error('po_bill_candidate_changed');
  ...
  return{run,row,candidate,realm:clean(realm),documentNumber,poNumber,qboPurchaseOrderId:clean(qboPurchaseOrderId),date,total,purchase,ap};
}
...
function buildPlan(source,purchaseOrder){
  ...
  const validPO=clean(purchaseOrder?.Id)===source.qboPurchaseOrderId&&clean(purchaseOrder.DocNumber)===source.poNumber&&clean(purchaseOrder.VendorRef?.value)===clean(source.candidate.qboVendorId)&&clean(purchaseOrder.POStatus).toLowerCase()==='open'&&Math.abs(money(purchaseOrder.TotalAmt)-source.total)<.005&&!poLinks.length;
  const validLines=poLines.length>0&&poLines.every(line=>clean(line.Id)&&line.DetailType==='AccountBasedExpenseLineDetail'&&clean(line.AccountBasedExpenseLineDetail?.AccountRef?.value)===clean(source.purchase.id)&&money(line.Amount)>0)&&Math.abs(money(poLines.reduce((sum,line)=>sum+money(line.Amount),0))-source.total)<.005;
  if(!validPO||!validLines)throw new Error('purchase_order_not_linkable');
  const lines=poLines.map(line=>({Amount:money(line.Amount),DetailType:'AccountBasedExpenseLineDetail',Description:`PO ${source.poNumber} — vendor doc #${source.documentNumber}`,LinkedTxn:[{TxnId:source.qboPurchaseOrderId,TxnType:'PurchaseOrder',TxnLineId:clean(line.Id)}],AccountBasedExpenseLineDetail:{AccountRef:{value:clean(source.purchase.id)},BillableStatus:'NotBillable',TaxCodeRef:{value:'NON'}}}));
  ...
  const summary={realm:source.realm,reviewRunId:clean(source.run.id),snapshotId:clean(source.run.snapshot_id),sourceHash:clean(source.run.report?.sourceHash),ledgerId:clean(source.row.id),documentNumber:source.documentNumber,vendor:clean(source.row.vendor),qboVendorId:clean(source.candidate.qboVendorId),date:source.date,total:source.total,poNumber:source.poNumber,qboPurchaseOrderId:source.qboPurchaseOrderId,purchaseAccount:{id:clean(source.purchase.id),number:'51300'},apAccount:{id:clean(source.ap.id),number:'21100'},posting:`${lines.length} account-based line${lines.length===1?'':'s'} linked to the existing QBO PO`,itemsCreated:0,inventoryQuantityPosted:false};
