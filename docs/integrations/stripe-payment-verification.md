...
Findings join the existing durable reconciliation incident/outbox pipeline. Healthy runs create no email. Open issues produce a consolidated email with the invoice IDs, payment reference and review reasons, with reminders limited by the existing nightly/23-hour deduplication window. A successful recheck resolves the issue. Digests containing invoice-payment or invoice-monitor issues go specifically to `steve@nationalsportsapparel.com`. Other existing payout-only digests retain their configured recipient routing. Delivery uses the existing Brevo configuration. No test email is sent by building or testing the PR.
