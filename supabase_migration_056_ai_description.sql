-- AI-cleaned product descriptions (lazy, saved once).
--
-- Vendor-scraped copy (esp. Adidas) reads like a spec sheet ("ZIPPER: ... WOVEN:
-- N/A HOOD: N/A ..."). netlify/functions/ai-clean-description rewrites it into clean
-- ecommerce copy with Claude (Haiku) and stores the result here, so each product is
-- cleaned at most once and the clean copy is reused everywhere.
--
--   • description_ai     — the AI-cleaned copy (NULL = not cleaned yet)
--   • description_ai_at  — when it was cleaned (audit / re-run gating)
--
-- The storefront view's `description` now prefers description_ai, falling back to
-- the raw description. Column name/type/position are unchanged (still text, last),
-- so this stays a drop-in CREATE OR REPLACE. The frontend still tidies raw copy as
-- a fallback for products not yet AI-cleaned.
--
...
ALTER TABLE products ADD COLUMN IF NOT EXISTS description_ai text;
ALTER TABLE products ADD COLUMN IF NOT EXISTS description_ai_at timestamptz;
