...
const source=fs.readFileSync('src/OrderMemoDialog.js','utf8').replace(/import React,\{([^}]+)\} from 'react';/, 'const React=window.React;const {$1}=React;').replace("from './lib/draftJournal'","from './lib/draftJournal.js'");
...
function App(){const [command,setCommand]=React.useState(null),[status,setStatus]=React.useState('Ready'),[offline,setOffline]=React.useState(false),[shownCloud,setShownCloud]=React.useState(cloud);
 ...
 command&&React.createElement(Dialog,{key:command.requestId||'new',initial:command,owner,journal,saveCommand:save,onClose:()=>{setStatus('Dialog closed; recovery kept unless acknowledged');setCommand(null);},onSaved:(id,memo)=>setStatus('Confirmed: '+memo)}));}
