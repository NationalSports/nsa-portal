-- Fix search_products RPC: exclude API-backed vendors (SanMar, S&S Activewear, Momentec)
-- from server-side search results so they only appear in their respective live API sections.
-- Also adds inventory_source to the return so the client-side filter can work correctly.
...
BEGIN
  ...
  WHERE (p_query IS NULL OR p_query = '' OR
    ...
    AND pr.inventory_source NOT IN ('sanmar', 'ss_activewear', 'momentec');
