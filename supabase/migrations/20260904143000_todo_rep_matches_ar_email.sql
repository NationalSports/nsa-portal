-- Attribute a past-due portal invoice to the same rep the A/R email does.
--
-- create_past_due_invoice_todos() resolved a portal invoice's rep as
-- coalesce(so.created_by, c.primary_rep_id) -- whoever raised the sales order.
-- rep-ar-digest.js resolves it as coalesce(i.rep_id, c.primary_rep_id,
-- i.created_by), because invoices.rep_id is the per-invoice override (see
-- commissionRepId in src/businessLogic.js) and, in that file's words, "it must
-- win here too or an overridden invoice chases the wrong rep for the money".
--
-- The two disagreed on 115 open invoices worth $288,282.05, so a rep's to-do and
-- their Friday email named different customers for the same debt. Worse, the
-- sales-order rule silently overrides account ownership: INV-63193 (Latah
-- Lightning Baseball, $2,949.00) belongs to the account's rep, but the order was
-- raised by someone else, so reassigning the account had no effect on the to-do.
--
-- Adopt the email's rule as the single one. The sales_orders join goes with it --
-- so.created_by was its only use here.
  ...
  for rep_record in
    with past_due as (
      select
        i.id as inv_id,
        i.customer_id,
        coalesce(i.rep_id, c.primary_rep_id, i.created_by) as rep_id,
