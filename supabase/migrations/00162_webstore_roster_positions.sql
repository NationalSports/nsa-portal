-- Bifurcate a club store's options by roster position (e.g. goalkeeper vs field
-- player) and expose it to the public storefront.
--
-- Soccer (and similar) kits differ by position: keepers get GK jerseys, gloves,
-- padded shorts; field players get the outfield kit; some items (team jacket,
-- spirit wear) go to everyone. Two small columns model that:
--
--   webstore_products.roster_audience — who an item is for: 'all' (default /
--     NULL), 'field', or 'gk'. Items tagged 'gk' show only to keepers, 'field'
--     only to outfield players; 'all' shows to everyone.
--   webstore_roster.position — the player's position: NULL (unassigned — sees
--     everything), 'field', or 'gk'. Set per player alongside their number.
--
-- When a player opens their roster link, the storefront filters the catalog to
-- items whose audience is 'all' or matches their position. Guests (no link) and
-- unassigned players see the full catalog and self-select.

ALTER TABLE public.webstore_products ADD COLUMN IF NOT EXISTS roster_audience text;  -- 'all' | 'field' | 'gk'
ALTER TABLE public.webstore_roster   ADD COLUMN IF NOT EXISTS position        text;  -- 'field' | 'gk'

-- Re-publish the storefront view with roster_audience so anon (which only reads
-- this SECURITY DEFINER view, never webstore_products) can filter by position.
-- Body is unchanged from 00158 except the one added column.
CREATE OR REPLACE VIEW webstore_storefront_products AS
 SELECT wp.id AS webstore_product_id,
    ...
    wp.kit_name,
    wp.card_style,
    COALESCE(wp.roster_audience, 'all') AS roster_audience
