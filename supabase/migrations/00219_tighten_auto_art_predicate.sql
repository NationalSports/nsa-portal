-- Audit fix (build audit, MEDIUM): tighten the auto-art birth predicate in BOTH
-- conversion RPCs so it matches the client's own rule.
--
-- THE BUG: v_auto_art (00207 create_teamshop_sales_order; 00214 create_club_sales_order)
-- births a job 'art_complete' when the saved-logo art is approved AND any of:
--   (a) prod_files_attached = true, OR
--   (b) prod_files array is NON-EMPTY, OR
--   (c) it's embroidery with a .dst file.
-- Arm (b) is too weak: ANY file in prod_files — e.g. a mock-up / order-sheet PDF that
-- staff left there with prod_files_attached still FALSE — births the job complete,
-- skipping the art/seps stage the client UI still shows it in. It also let a job whose
-- own deco_type is embroidery skip digitizing off a non-embroidery art's stray file.
--
-- THE FIX: drop arm (b). The predicate now mirrors the client's canonical
-- artProdFilesConfirmed (src/constants.js:360) EXACTLY: confirmed only on an explicit
-- prod_files_attached=true, or an approved embroidery art carrying a .dst. This is the
-- ONLY change from the live bodies (create_teamshop_sales_order from 00207,
-- create_club_sales_order from 00214) — extracted verbatim minus that one OR arm.
--
-- Rollback: re-apply create_teamshop_sales_order from 00207 and create_club_sales_order
-- from 00214.
