-- Audit fix (build audit, MEDIUM): scope create_club_sales_order's
-- webstore_bundle_items reads to the ORDERING store's bundles.
--
-- THE BUG (00204 original + 00207 live body, both applied to prod): the club
-- conversion resolved bundle-component personalization (takes_number/takes_name)
-- and transfer_codes with `where product_id = v_grp.product_id` and NOTHING
-- else. webstore_bundle_items rows belong to a bundle (webstore_products) in a
-- SPECIFIC store, but products.id is a GLOBAL stock-catalog key shared across
-- stores — so any OTHER store's bundle rows for the same stock product were
-- unioned in. A stock jersey sold plainly in club store A, that also happens to
-- be a component of team store B's coded bundle, got B's transfer_code welded
-- onto A's SO: a phantom heat_press deco + a 'press this transfer' production
-- job the buyer never ordered, and — on a staff-entered same-code collision in
-- store A's own webstore_transfers — a real GP/commission miscost.
--
-- THE FIX: both reads now additionally require
--   bundle_id in (select id from webstore_products where store_id = <order's store>)
-- exactly mirroring the client path this RPC claims parity with (Webstores.js
-- filters bundleItems by this store's webstore_products / catIds). Non-bundle
-- club orders are unaffected (no bundle_items rows match). This is the ONLY
-- change from the 00207 body — extracted verbatim and re-emitted so CREATE OR
-- REPLACE supersedes the live function; privileges are preserved by REPLACE.
--
-- Rollback: re-apply the create_club_sales_order block from 00207_auto_art.sql.
