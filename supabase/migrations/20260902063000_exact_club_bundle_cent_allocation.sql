-- Exact club bundle cent allocation (20260902063000).\n--\n-- Carries forward the current 00235 club-conversion body and changes only the\n-- bundle allocation scratch: integer cents + largest remainders guarantee that\n-- component collected values sum exactly to the paid bundle parent.\n--\n-- Club transfer auto-art behavior from 00235 remains intact.
--
-- GOAL (owner's target state): club store orders need NO art step at all — only
-- item fulfillment, applying decoration, and shipping. Auto-art (00207/00219)
-- already births 'art:<id>' logo jobs art_complete when the customer's art-library
-- entry is approved with production files. But transfer jobs ('xfer:<code>') were
-- deliberately excluded — they were born needs_art and sat in the art queue even
-- though a heat-transfer design IS finished art: the physical transfer either sits
-- in webstore_transfers inventory or is on order from the supplier; there is
-- nothing for an artist to do at order time.
--
-- WHAT CHANGES (club RPC only; create_teamshop_sales_order is untouched):
--   In the so_jobs birth loop, a job whose logo_ref is 'xfer:<code>' is born
--   art_status='art_complete' IFF a webstore_transfers row exists for this store
--   and code (v_xfer_ready). The existence check is the guard: a catalog
--   transfer_code with NO matching inventory row (mistyped, deleted, never
--   created) keeps today's needs_art birth, which keeps a human in the loop for
--   exactly the broken-config case. art_file_id/_art_ids stay NULL/[] (there is
--   no art-library file for a transfer); the job name already carries the
--   transfer label from _ts_job_decos.
--
--   The 'created' job_stage_event stamps auto_art=true for these births (plus a
--   new xfer_ready flag for auditability), so the 00208 auto-release sweep's
--   conservative 'auto_art_only' scope covers transfer jobs too: once garments
--   are in hand, the sweep releases them with zero staff touches. The sweep's art
--   gate only inspects FOUND art records (a transfer job references none), and
--   its DTF-prints hold only applies to deco_type='dtf' — 'heat_press' jobs pass.
--
-- Body carries forward the live 00235 create_club_sales_order implementation,
-- including transfer auto-art, and changes only bundle collected-value allocation.
-- Rollback: re-apply create_club_sales_order from 00235.
...
declare
  ...
  v_art_id      text;
  v_art_entry   jsonb;
  -- ── Transfer auto-art (00235) ──
  v_xfer_ready  boolean;
begin
  ...
  -- Allocate integer cents with largest remainders. Independent per-child
  -- rounding made $100 / 3 become $99.99; this assigns the residual penny
  -- deterministically while preserving the existing retail-weighted split.
  drop table if exists _club_bundle_alloc;
  create temporary table _club_bundle_alloc (item_id uuid primary key, collected numeric) on commit drop;
  insert into _club_bundle_alloc (item_id, collected)
  with weighted as (
    select i.id as item_id, i.bundle_product_id as bpid,
           round(coalesce(bp.parent_val, 0) * 100)::bigint as total_cents,
           case when coalesce(bw.wsum, 0) > 0 then coalesce(p.retail_price, 0) else 1 end as weight,
           case when coalesce(bw.wsum, 0) > 0 then bw.wsum else greatest(coalesce(bw.n, 1), 1) end as weight_sum
      from webstore_order_items i
      left join products p on p.id = i.product_id
     ...
     where i.order_id = v_ord.id
       ...
       and i.bundle_product_id is not null
  ), raw as (
    select *, total_cents * weight / nullif(weight_sum, 0) as raw_cents
      from weighted
  ), based as (
    select *, floor(raw_cents)::bigint as base_cents,
           raw_cents - floor(raw_cents) as fraction
      from raw
  ), ranked as (
    select *, total_cents - sum(base_cents) over (partition by bpid) as residual_cents,
           row_number() over (partition by bpid order by fraction desc, item_id) as residual_rank
      from based
  )
  select item_id,
         (base_cents + case when residual_rank <= residual_cents then 1 else 0 end)::numeric / 100
    from ranked;
  ...
  select i.id, i.product_id, i.sku, i.size, coalesce(i.qty, 1), i.player_name, i.player_number,
    case
      when i.bundle_product_id is not null then coalesce(ba.collected, 0)
      else round(((coalesce(i.unit_price, 0) + coalesce(i.unit_fundraise, 0)) * coalesce(i.qty, 1))::numeric, 2)
    end
    from webstore_order_items i
    left join _club_bundle_alloc ba on ba.item_id = i.id
  ...
  loop
    ...
    select bool_or(coalesce(takes_number, false)), bool_or(coalesce(takes_name, false))
      ...
      from webstore_bundle_items
     where product_id = v_grp.product_id
       and bundle_id in (select id from webstore_products where store_id = v_ord.store_id);
  ...
  loop
    ...
    -- ── Transfer auto-art (00235) ── a heat-transfer design is finished art: the
    -- physical transfer is stocked (or reordered) in webstore_transfers, so there
    -- is no art work to do. Born art_complete IFF the store actually has a
    -- transfer row for this code — a missing row (mistyped/deleted code) keeps
    -- the needs_art birth so a human reviews the broken config.
    v_xfer_ready := v_job.logo_ref like 'xfer:%'
      and exists (
        select 1 from webstore_transfers t
         where t.store_id = v_ord.store_id
           and t.code = substring(v_job.logo_ref from 6));
    ...
    insert into so_jobs (
      ...
      case when v_auto_art or v_xfer_ready then 'art_complete' else 'needs_art' end,
      'need_to_order', 'hold', v_job_units, 0, null, v_today_txt,
      'ship_customer', coalesce(v_job_items, '[]'::jsonb), true,
      case when v_auto_art or v_xfer_ready then false else v_job.digitizing end
    );
    ...
    values (
      ...
      jsonb_build_object('prod_status', 'hold',
                         'art_status', case when v_auto_art or v_xfer_ready then 'art_complete' else 'needs_art' end),
      null, 'club',
      jsonb_build_object('webstore_order_id', v_ord.id, 'logo_ref', v_job.logo_ref,
                         'digitizing_needed', case when v_auto_art or v_xfer_ready then false else v_job.digitizing end,
                         'auto_art', v_auto_art or v_xfer_ready,
                         'xfer_ready', v_xfer_ready,
...
-- Service-role only — same posture as 00204 (create or replace preserves ACLs;
-- re-issued defensively).
