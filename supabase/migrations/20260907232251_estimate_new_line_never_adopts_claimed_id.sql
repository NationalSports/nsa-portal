begin;
-- A line added to an estimate leaves the client without a line_id (only Copy Item minted one).
-- save_estimate's legacy matcher then gave it the line_id of the EXISTING line with the same
-- SKU/color/product, the upsert hit that row twice, and the save was rejected with
-- ESTIMATE_DUPLICATE_LINE_ID — every attempt, forever (EST-2434: a second "R095ZM / Default"
-- line; 47 rejections in 24h across the retry loop). Ids the payload names explicitly are now
-- off-limits to the matcher, and an id a blank line adopts is claimed for the rest of the save,
-- so a genuinely new line gets a fresh uuid. Offline drafts with a unique, unclaimed twin still
-- re-attach exactly as before; the DUPLICATE_LINE_ID guard stays as the last line of defence.
...
BEGIN
  ...
  IF jsonb_typeof(p_items) = 'array' THEN
    ...
    END IF;
    -- Ids the payload names explicitly. A line WITHOUT one must never adopt them: it is a
    -- new line that happens to share SKU/color with an existing one (EST-2434), not the
    -- offline-draft twin of that row. Adopting made the upsert hit the same row twice.
    SELECT coalesce(array_agg(nullif(value->>'line_id','')),ARRAY[]::text[]) INTO v_claimed
    FROM jsonb_array_elements(p_items) WHERE nullif(value->>'line_id','') IS NOT NULL;
    ...
    LOOP
      ...
      IF v_line_id IS NULL THEN
        ...
        WHERE coalesce(x->>'sku','')=coalesce(v_item->>'sku','')
          ...
          AND NOT ((x->>'line_id') = ANY(v_claimed));
