-- Generic audit log + triggers for every critical app table (hardened).
--
-- Supersedes 00070_audit_log.sql, which never applied successfully: it attached
-- triggers to a hardcoded table list with no existence check, so the first table
-- that didn't exist in this database (price_matrix, decoration_types) raised
-- `relation "public.<t>" does not exist` and aborted the whole migration. As a
-- result no audit_log table or triggers were ever created — which is why a
-- deleted invoice (e.g. the one on SO-1088) left no record at all.
--
-- This version guards each attach with to_regclass(), so missing tables are
-- skipped instead of aborting, and the migration is fully idempotent.
--
...
-- when, and by whom?" — and recover row contents on demand.
...
-- ─── Attach to every critical table that exists ────────────────────────────
-- Idempotent + existence-guarded: a table in the list that doesn't exist in
-- this database is skipped (logged via raise notice) instead of aborting the
-- migration. Re-running picks up any table created later.
do $$
declare
  t text;
  audited text[] := array[
...
begin
  foreach t in array audited loop
    if to_regclass(format('public.%I', t)) is null then
      raise notice 'audit_log: skipping % (table does not exist)', t;
      continue;
    end if;
